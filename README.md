...
这是从 HIFM v0.3 迁入的通用执行指南（`0.3.1-draft.7`，待完整写作任务验证），不是新模板，
不改变项目已采用的 STD/模板版本，也不自动授权提交、发布或运行。

当前工作区的 `design.system` 为待发布的 `6.3.0`：沿用 `6.0.0` 的 Markdown 短封面，正文前仅保留八项
...
保留原始 PNG 与生成提示词，另附简洁 SVG 框图。图件均为原创虚构教学素材，不宣称整套
示例已经补齐。明确标记的教学段落在 `new-design`
