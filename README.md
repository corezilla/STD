...
这是从 HIFM v0.3 迁入的通用执行指南（`0.3.1-draft.12`，待完整写作任务验证），不是新模板，
...
当前工作区的 `design.system` 为待发布的 `7.0.0`，目录与 HIFM 当前系统设计的通用章节顺序对齐。
...
目录为 17 个主章，属于不兼容结构调整，因此独立提升系统模板主版本；其他模板不连带升级。
