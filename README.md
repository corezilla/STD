...
当前工作区的 `design.system` 为待发布的 `5.1.0`：保留 18 个主章、必需系统概览与
...
兼容增补，因此已提升主版本。本轮在 `5.0.0` 上补充客户业务场景、章节预设计、系统约束分配、
制造测试预留及统一收口检查，保留现有章节编号，按兼容补充提升至 `5.1.0`。
`design.definition` 与 `design.system-mechanism` 分别补充上级约束承接入口，独立升为待发布
`1.1.0`；系统保留端到端原理与关键阶段，机制文档唯一维护详细状态转换及参与方协议。
