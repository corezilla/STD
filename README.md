...
`design.definition` 为 `3.3.0`《软件模块设计说明书》，保留 15 个主章；服务型模块可把 HTTP/RPC 端点集登记为操作面，§5–7 显式展开内部组成、文件调用、文件间接口与字段级数据结构，§13 固定文件分解和实现步骤，§14.1 从适用约束、功能、过程、规则、接口与错误正向核对正文决定、实现责任和验证。常驻、跨步骤状态与持久写入按事实触发生命周期、状态模型及恢复设计；附录 A 仍以 Mechanism Document ID + Requirement ID 精确承接机制 §14.4。复杂度来自模块事实，不按篇幅分档；旧采用实例不自动升级。
配套子系统与模块 AI 指南为 `0.13.0-draft.6`，增加模块试写流程、历史问题复发检查、正向覆盖评审清单、[异步模块连续案例](docs/examples/module-async-export-example.md)以及经授权的 [LLMTier M001/M002 实证样板](demo/llmtier-module-design/README.md)。新版模块生成限制软件模块层级，并保留内容寻址的教学参考；构建装配、生命周期预算及四类恢复动作分别核对。旧采用实例不自动升级。
