...
[`docs/ai-system-design-authoring-guide.md`](docs/ai-system-design-authoring-guide.md)（`0.7.0-draft.9`）；
...
`design.definition` 为 `3.4.0`《软件模块设计说明书》，保留 15 个主章；服务型模块可把 HTTP/RPC 端点集登记为操作面，§5–7 显式展开内部组成、文件调用、文件间接口与字段级数据结构，§13 固定文件分解和实现步骤，§14.1 按单个来源 ID 从适用约束、功能、过程、规则、接口与错误正向核对正文决定、实现责任和验证。接口核对上级与提供方的签名及数据来源；状态转换的持久动作须有可调用的原子操作。常驻、跨步骤状态与持久写入按事实触发生命周期、状态模型及恢复设计；附录 A 仍以 Mechanism Document ID + Requirement ID 精确承接机制 §14.4。复杂度来自模块事实，不按篇幅分档；旧采用实例不自动升级。
配套子系统与模块 AI 指南为 `0.14.0-draft.1`，增加模块试写流程、历史问题复发检查、跨文档接口合同核对、正向覆盖评审清单、[异步模块连续案例](docs/examples/module-async-export-example.md)以及经授权的 [LLMTier M001/M002 实证样板](demo/llmtier-module-design/README.md)。新版模块生成限制软件模块层级，并保留内容寻址的教学参考；构建装配、生命周期预算及四类恢复动作分别核对。旧采用实例不自动升级。
`design.hardware` 与 `design.fpga` 增加上级约束承接、
本地落实和系统组合验收，当前均为 `1.4.0`，不要求另建通用单元文档。
`design.system-mechanism`（总体/纯软件系统机制设计）为 `3.3.0`，数据与接口分章，16 个主章保留既有约束和段落式指导，编写指令与教学内容隔离于项目正文；§1、§3、§6 明确基线图，其他图按条件触发并记录裁剪依据；§5 对每个重要接口强制用代表输入完成调用演练，§14 固定为参与方映射、步骤责任、跨单元接口和下级设计输入四段式要求侧，纯软件 API 裁剪示例放附录 A；下级要求、风险和约束在对应章节使用不同 ID 命名空间。
...
[机制 AI 专项指南](docs/ai-guides/system-mechanism.md)补充双方调用演练、异常五轴、条件依赖、验证承接和全篇一致性检查；机制内容复核还需从跨单元交接反查接口、用反例验证关键保证，并明确事实 authority、预算及独立判据，章节和图表齐全不等于设计闭合。
