...
版本：0.6.0-draft.3 · 日期：2026-09-12 · 状态：编写方法草案，待完整写作任务验证
...
本轮修订输入基线：STD commit `b09c442ca6653896e445c907c6e8e3ccf62a791d`，包含系统模板
`8.2.0`、机制模板 `2.1.0` 和本指南 `0.6.0-draft.2`。本版仅同步下游模板映射，保留系统 17 章方法；
机制模板 `2.2.0` 的 16 章写法、九幅图文样板、软硬件案例及批量复审
...
| 方法来源 | 本版核对版本 | 在写作中的用途 |
|---|---|---|
| `design.system` / `architecture-design.md` | 8.2.0（待发布） | 17 主章；机制清单含上级 ID、独立前置依赖与预定文件映射 |
| `design.definition` | 1.3.0 | 固定上级约束和接口成员基线到实现/验证；易失状态也需生命周期 |
| `design.hardware`、`design.fpga` | 各 1.2.0 | 信号/寄存器和系统分配使用同一标识及固定来源 |
| `design.system-mechanism` | 2.2.0（待发布） | 16 主章；承接机制父子身份，保留数据/接口、机器契约及下游映射 |
...
`design.system-mechanism` 的主专项现为[系统机制 AI 编写指南](ai-guides/system-mechanism.md)，包含 16 章写作顺序、九幅可编辑图例、完整调用案例、能力覆盖和双方承接检查。原入口保留导航，不再维护另一份机制方法。
