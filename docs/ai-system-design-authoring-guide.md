...
版本：0.7.0-draft.5 · 日期：2026-09-24 · 状态：编写方法草案，待完整写作任务验证
...
| 方法来源 | 本版核对版本 | 在写作中的用途 |
|---|---|---|
| `design.system` / `architecture-design.md` | 8.4.0 | 总体系统设计，17 主章；三条领域分支、公共错误码及横向机制映射 |
| `design.definition` | 2.5.0 | 软件模块详细设计；端点操作面、内部调用/接口、字段数据、文件分解、重要过程图与机制承接固定记录及ISD采用登记；机制要求按稳定行 ID 精确承接 |
| `design.implementation` | 0.6.0 | 软件模块实现视图；条件化 Current、函数输入/输出/错误契约、流程图、配置落点、硬约束与实现自由度、并发、持久化/schema拒绝语义及独立 Oracle |
| `design.hardware`、`design.fpga` | 各 1.3.0 | 信号/寄存器和系统分配使用同一标识及固定来源 |
| `design.system-mechanism` | 2.7.0 | 总体/纯软件系统机制设计，16 主章；横向承接，保留数据/接口、机器契约及跨责任单元分解；代表输入必须完成调用演练，完整教学案例外移；要求/风险/约束 ID 分域 |

这些模板改进随 STD `0.1.0-draft.32` 发布。这里记录的是**编写方法来源和版本映射**，不是宣布使用本指南的项目已升级模板锁或批准产品设计。执行实际写作任务时，应核实项目已采用版本与任务授权，不主动查询最新版本；有差异就记录适用关系，不擅自修改 `std.lock`、模板元数据或项目采用状态。本文的“STD §…”按上表版本映射；“本指南第…章”指本文。
