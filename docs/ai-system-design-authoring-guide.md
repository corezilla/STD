...
| `design.software-system` | 1.1.0 | 软件系统设计；继承总体约束，定义业务数据、公共边界与直接下级 |
...
这些模板改进对应 STD `0.1.0-draft.36`。这里记录的是**编写方法来源和版本映射**，不是宣布使用本指南的项目已升级模板锁或批准产品设计。执行实际写作任务时，应核实项目已采用版本与任务授权，不主动查询最新版本；有差异就记录适用关系，不擅自修改 `std.lock`、模板元数据或项目采用状态。本文的“STD §…”按上表版本映射；“本指南第…章”指本文。
