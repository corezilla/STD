...
保留总体系统 17 章方法；总体系统机制的 16 章写法、九幅图文样板、软硬件案例及批量复审
...
| `design.system-mechanism` | 3.2.0 | 总体/纯软件系统机制设计，16 主章；横向承接，保留数据/接口、机器契约及跨责任单元分解；代表输入必须完成调用演练，完整教学案例外移；要求/风险/约束 ID 分域 |

这些模板改进对应 STD `0.1.0-draft.39`。这里记录的是**编写方法来源和版本映射**，不是宣布使用本指南的项目已升级模板锁或批准产品设计。执行实际写作任务时，应核实项目已采用版本与任务授权，不主动查询最新版本；有差异就记录适用关系，不擅自修改 `std.lock`、模板元数据或项目采用状态。本文的“STD §…”按上表版本映射；“本指南第…章”指本文。
...
`design.system-mechanism` 的主专项现为[总体系统机制设计 AI 编写指南](ai-guides/system-mechanism.md)，包含 16 章写作顺序、九幅可编辑图例、完整调用案例、能力覆盖和双方承接检查。原入口保留导航，不再维护另一份机制方法。
