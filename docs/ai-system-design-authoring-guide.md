...
版本：0.7.0-draft.8 · 日期：2026-09-25 · 状态：编写方法草案，待完整写作任务验证
...
| `design.definition` | 3.3.0 | 软件模块详细设计；端点操作面、内部调用/接口、字段数据、文件分解、重要过程图、条件状态模型与机制承接固定记录及ISD采用登记；§14.1 正向核对适用 ID 到实现与验证 |
...
这些模板改进对应 STD `0.1.0-draft.42`。这里记录的是**编写方法来源和版本映射**，不是宣布使用本指南的项目已升级模板锁或批准产品设计。执行实际写作任务时，应核实项目已采用版本与任务授权，不主动查询最新版本；有差异就记录适用关系，不擅自修改 `std.lock`、模板元数据或项目采用状态。本文的“STD §…”按上表版本映射；“本指南第…章”指本文。
