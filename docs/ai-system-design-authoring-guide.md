# 总体系统设计 AI Agent 编写指南：从产品场景到可评审、可承接的完整设计

版本：0.7.0-draft.9 · 日期：2026-09-27 · 状态：编写方法草案，待完整写作任务验证
...
阅读分工：[模板选择规则](template-selection.md)决定使用什么模板；[设计文档编写规范](design-writing-guide.md)规定共同质量要求；[总体系统设计写作与评审指南](architecture-design-authoring-guide.md)解释内容深度；本文说明 AI 如何把这些要求转成计划、设计、成文和检查。正文承载结构仍以项目已采用的模板为准。
...
当前名称为“总体系统设计”，Template ID/文件路径不变。按[分层规则](template-selection.md#2-分层设计)
统筹软件系统、固件系统与硬件系统；软件系统不是一级软件子系统。软件子系统只分到软件模块，
当前不递归；板卡和 FPGA 程序沿各自专项分解。总体系统机制设计横向承接共同协议，不是固定分解层。
软件系统采用 `design.software-system` 和[软件系统专项](ai-guides/software-system.md)；纯软件顶层不强制另写重复的总体设计，领域模式承接本层分配。
...
本轮修订输入基线：STD commit `7fb4af53b88ec508e0e5fe4452ebcd8ba673253f`，包含系统模板
`8.2.0`、机制模板 `2.2.0` 和本指南 `0.6.0-draft.3`。本轮统一总体名称和三条设计分支，
保留总体系统 17 章方法；总体系统机制的 16 章写法、十幅图文样板、软硬件案例及批量复审
入口见[机制专项](ai-guides/system-mechanism.md)。公共规则继续由通用指南维护，本轮不再拆分指南。
机制模板和本指南的新修订以各自版本、实际摘要及所在提交为准，不倒称已包含在输入
基线或旧发布标签中。下表是系统写作及下游承接所参照的模板关系，不表示这些模板都以本文作为主专项指南：

| 方法来源 | 本版核对版本 | 在写作中的用途 |
|---|---|---|
| `design.system` / `architecture-design.md` | 10.1.0 | 总体系统设计，17 主章；三条领域分支、公共错误码、横向机制映射及 §10.4 的 CLI/WebUI 入口设计 |
| `design.software-system` | 2.1.0 | 软件系统设计；§3.6 定义功能，继承总体约束并定义业务数据、公共边界与直接下级 |
| `design.subsystem` | 1.2.0 | 软件子系统设计；定义本层组织、数据变换和模块交接 |
| `design.definition` | 3.4.0 | 软件模块详细设计；端点操作面、内部调用/接口、字段数据、文件分解、重要过程图、条件状态模型与机制承接固定记录及ISD采用登记；核对跨文档接口签名与状态动作；§14.1 按单个来源 ID 正向核对到实现与验证 |
| `design.implementation` | 1.2.0 | 软件模块实现视图；条件化 Current、函数输入/输出/错误契约、流程图、配置落点、硬约束与实现自由度、并发、持久化/schema拒绝语义及独立 Oracle；图例提供 SVG 展示与可折叠 Mermaid 源码 |
| `design.hardware`、`design.fpga` | 各 1.4.0 | 信号/寄存器和系统分配使用同一标识及固定来源 |
| `design.system-mechanism` | 3.3.0 | 总体/纯软件系统机制设计，16 主章；横向承接，保留数据/接口、机器契约及跨责任单元分解；代表输入必须完成调用演练，图件区分基线与条件并记录省图依据；要求/风险/约束 ID 分域 |

这些模板改进对应 STD `0.1.0-draft.43`。这里记录的是**编写方法来源和版本映射**，不是宣布使用本指南的项目已升级模板锁或批准产品设计。执行实际写作任务时，应核实项目已采用版本与任务授权，不主动查询最新版本；有差异就记录适用关系，不擅自修改 `std.lock`、模板元数据或项目采用状态。本文的“STD §…”按上表版本映射；“本指南第…章”指本文。
...
| 跨单元机制 `design.system-mechanism` | §3.1；§15 验证 | 各方保证与前提能接起来，详细协作不改变系统政策 |
| 子系统 `design.subsystem` | §2–5、§13–15 | 整体概要方案落实为模块与运行设计、共享预算闭合、整体能力的组合验收 |
...
写入 STD 模板时注意分工：§10.4 按真实入口逐项定义维护/调试接口，包括适用的指示、查询、命令、替代依赖、旁路和环回；§11.2 定义统计、日志与故障定位语义，§11.2.1 写自检/诊断；§11.3 写升级/回滚；§13 使用这些能力设计验证，不重新定义一套接口。维护机上的远程客户端、目标节点本地命令和串口操作要分清，不能写一个命令名就默认任何机器均可执行。
...
系统模板 §3.4 在总体设计时先登记机制 ID、上级 Mechanism ID、Document ID、预定文件名、范围、Owner/前置依赖与写作状态。
归属树与依赖图分别走查：顶层 none，直接上级可为 Planned 但须已登记，禁止自指、循环、悬空；多处使用不新建同义机制。示例 M-EXPORT 属于 M-DELIVERY，却可以有跨分支写作前置 M-OBS；依赖不表示归属，也不自动新增运行调用。
先建立映射再逐份写，Planned 路径用代码文字且已成文版本 none；写成后改真实链接并核对文档身份/基线。
系统仍解释原理及代表失败，不因目录齐全便省略正文或宣称下游可实现。§9/10 接口与数据共用
[映射规范](interface-data-mapping-standard.md)的稳定成员 ID，连接实际契约、正文与下游验证。
完整路径演练见[暂存导出映射案例](examples/interfaces/README.md)，不可把预定机制文件注册为已存在机器源。

### 14.1 系统机制写作入口（兼容导航）

`design.system-mechanism` 的主专项现为[总体系统机制设计 AI 编写指南](ai-guides/system-mechanism.md)，包含 16 章写作顺序、十幅可编辑图例、完整调用案例、能力覆盖和双方承接检查。原入口保留导航，不再维护另一份机制方法。
...
- [STD 通用单元模板](../templates/design/design-definition.md)、[总体系统机制设计模板](../templates/design/system-mechanism-design.md)。
...
用户的明确要求，不能从指南或 README 更新推导项目已升级；本轮同步模板分层规则与 AI 导航，
不更改项目 lock，也不发布新的 STD 版本或 RAG 清单。
