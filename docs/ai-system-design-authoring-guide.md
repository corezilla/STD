...
| 方法来源 | 本版核对版本 | 在写作中的用途 |
|---|---|---|
| `design.system` / `architecture-design.md` | 8.3.1（待发布） | 总体系统设计，17 主章；三条领域分支及横向机制映射 |
| `design.definition` | 2.3.0 | 软件模块详细设计；逐章指导、重要过程图、机制承接表、连续案例与约束/接口/实现/验证承接及ISD采用登记；机制要求按稳定行 ID 精确承接 |
| `design.hardware`、`design.fpga` | 各 1.2.1 | 信号/寄存器和系统分配使用同一标识及固定来源 |
| `design.system-mechanism` | 2.5.0（待发布） | 总体/纯软件系统机制设计，16 主章；横向承接，保留数据/接口、机器契约及跨责任单元分解；代表输入必须完成调用演练，完整教学案例外移 |
