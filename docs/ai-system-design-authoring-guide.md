...
| `design.definition` | 2.2.0 | 软件模块详细设计；逐章指导、重要过程图、机制承接表、连续案例与约束/接口/实现/验证承接及ISD采用登记 |
| `design.hardware`、`design.fpga` | 各 1.2.1 | 信号/寄存器和系统分配使用同一标识及固定来源 |
| `design.system-mechanism` | 2.4.0（待发布） | 总体/纯软件系统机制设计，16 主章；横向承接，保留数据/接口、机器契约及跨责任单元分解；编写说明与正文隔离 |
