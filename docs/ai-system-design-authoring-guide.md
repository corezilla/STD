...
版本：0.4.0-draft.1 · 日期：2026-09-11 · 状态：编写方法草案，待完整写作任务验证
...
本轮修订输入基线：STD commit `ceaeb5f79e798f6b8139b8ae9bc6d2aefadf8155`，包含系统模板
`7.0.0` 和本指南 draft.12。本版对应同目录的待发布系统模板 `8.0.0`：保留 17 个主章及图形
样式，补齐通用协作、公共契约、诊断与测试控制子节；区分待设计与待实现，按能力检查双方承接。
`8.0.0` 及本指南修订以各自版本、实际文件摘要和所在提交为准，不倒称已包含在输入
基线或旧发布标签中。下表为本版对应关系：
...
| `design.system-mechanism` | 1.3.0（待发布） | 运行时统筹、参与方确认、公共契约、未知结果及安全恢复 |
...
预设计评审或自审后，把获得依据且符合决定权限的方案、原理、限制、失败行为和验证方法写回系统正文，并把需要下游遵守的部分写入 STD §3.2。重要选择依项目 ADR/批准规则处理，自审不能替代批准。一张必要的关系图或配置表，加上能说明原理和取舍的几段正文，通常就能表达重点。关键前提未定的部分保留影响、可继续范围和待决项；具体安装命令、测试 Case、数据生成脚本和运行证据由下级承接，不全部复制到系统设计。
...
省略或合并章节时，沿用所选模板的适用矩阵和项目已有 tailoring 决定，保留批准依据及信息项去向；待批裁剪不是已经免除要求。局部编辑不重新打开未受影响的裁剪决定。
...
**区分两份计划。** 本节是 Agent 为产出文档而制定的写作计划；STD §16 是产品由设计走向实现和验证的工程计划。前者的“写完接口说明”不能冒充后者的“接口已实现并通过测试”。工程计划应先评审接口语义、错误行为、Schema/向量，再允许两端并行实现；各自契约测试通过后进入集成，再以端到端证据关闭系统目标。不得要求尚未实现的两端先通过实现测试才能开始开发。
...
**第四步，先表达主要关系。** 软件分层组成图先排列层次与模块，左侧写层名，用包含关系表达归属，不画图标或连接线。这不禁止系统总图、FPGA 主数据路径或板卡图表达必要连接。交互图先画最重要路径，将业务主方向保持为左到右或上到下，再补返回、控制和异常关系。模块多时分组或拆图，不靠缩小字体塞入更多方框。
...
新画软件、FPGA 程序和板卡组成时，优先按 §7.9 复用原生 SVG；已有合格图保留其维护源，不为统一工具而重画。其他职责框图、流程、时序和状态可用 Mermaid，便于与正文一起修改。精细部署、结构、数据布局及复杂排版可用可编辑 SVG、绘图工具或原生文档形状；需要强调使用环境、角色或空间关系时再使用说明性插画或获准的照片。

工程图不能把生成式图片中的线、端口和文字当作技术依据。概念图可以帮助记忆产品，但不能代替带方向、接口及边界的工程图；保密来源不得发送给外部绘图或图像生成服务。资产应使用原创、已授权或允许使用的来源。

原生工程图保留 SVG、Mermaid 或绘图工具源，不只保留截图；生成式插画保留原始位图及允许保存的提示词、来源和边界说明，不声称具有不存在的 SVG/三维源，也不承诺重新生成像素一致。没有权限保存的参考内容不进入生成记录。图号和文件名稳定，图注与正文引用同步更新。在 STD 目录中，共用文档小型图件可放 `docs/assets/`，临时渲染和运行产物放项目规定的数据位置，不随意新建并列资产目录。
...
2. **选表达与工具。** 新图或获准更换样式的图先按 §7.9 选择图源；软件、FPGA 程序和板卡图直接编辑对应 SVG，不重新生成一套外观。既有合格图不因阅读本指南就强制转换格式。其他精确连接、状态和时序可用 Mermaid 或项目已有绘图方式；少量精确映射用表更清楚就不画。场景插画可以使用获准的图像工具，但不能把其自动生成的连线、端口或文字当工程事实。按所用工具的适用技能和权限操作，不假设某插件一定存在。
...
6. **同步维护。** 变更 Owner、部署、流程或预算时按图卡找到受影响图，重新检查；按 §7.3 保留相应图源或原始位图记录及可阅读形式。不需要把每次生成的废稿全部放进正式资料。
...
分开记录四件事：文档是否获批、图描述的是现状还是目标、能力实现到哪里、在什么条件下验证到哪里。按对应系统模板 §B.3 使用以下词表，不把它们排成一条升级流水线：

- 文档状态按项目文档控制规则记录，如 Draft、In Review、Approved。
- 架构视图为 `Current / Target / Transitional`。
- 实现状态为 `Planned / Partial / Implemented`。
- 验证结果为 `NOT_RUN / BLOCKED / FAIL / PARTIAL / Verified`，绑定同一能力范围、输入版本、配置和验收条件；工具原始 PASS/FAIL 结果原样保留。

例如代码已存在但尚未测过，写 Implemented / NOT_RUN；只测过一个子功能，不能把整个模块写成 Verified。文档 Approved 不意味着硬件或性能 Verified；结构校验 PASS 不意味着设计合理。
...
交稿包应能找到：唯一的系统设计正文；原生工程图的可编辑源，或生成式插画的原始位图与获准生成记录，以及已检查的展示形式；固定来源与必要的预设计；约束分配及下游承接/验证计划；已完成工作包、内容检查结果及仍未解决的问题。局部交稿只报告本次变更及关联检查。优先复用正文、计划和项目已有记录，不为每项都新增文件。
...
> 你是负责系统设计成文的 AI Agent。目标是让读者理解产品，并让下游知道必须遵守什么、可以自行设计什么。初次完整编写先完整读《系统设计 AI Agent 编写指南》和选定模板的帮助；续写或局部修订先读已有记录、适用章节和差异，再核查项目事实与修改边界；不能把模板示例当产品事实。
>
> 按 §4.1 区分完整编写与局部修订，把所需步骤实例化为工作包，记录设计问题、输入/依赖、正文和图件路径、完成检查、状态及下一步。计划建立后执行，不止于交计划；允许分多轮完成，不要求一次生成全文。局部任务不重跑整套 S0～S7，完成授权范围后交付。写作计划与产品实现计划分开。保存续写信息，每轮恢复先核实当前文件，不覆盖用户新修改。
>
...
>
> 每个关键节产出具体决定、依据、下游约束、自由度及检查方法。按 STD §3.2 汇总分配，按领域由机制、通用单元或硬件/FPGA 专项模板直接承接；不要复制权威契约或重复建文。共享预算只计一次，行为保证逐跳相接，本地通过不等于系统组合通过。
>
> 按 §4.8 的能力工作包贯通场景、运行时统筹、参与方确认、完整公共对象/操作/错误及双方验收。
> 打开实际技术定义检查，不只检查路径存在。已有局部设计先整合，冲突先明确裁决，缺少共同
> 方案先预设计；不能将它们统称“下级细化”。子节用通用规则、适用条件和虚构例子指导，不
> 从示例复制产品参数或新增机制；参考 §8.8 的收口深度，而不是照搬该协议。
>
...
>
> 按第 7 章为必要插图建立图卡，确定对象、边界、关系、状态和来源后绘制。新图按 §7.9 选用已有图形样式：软件、FPGA 程序和板卡图复制对应 SVG 到项目资产目录，直接替换内容、调整分组与连线；场景和轻量系统图仅作为位图风格参考。既有合格图不强制换格式。复用样式不继承示例设计，不主动升级标准；视图、实现和验证状态分开填写。按 §7.3 保留实际存在的图源或原始位图与获准记录，在实际阅读宽度查看渲染结果，再补图注和逐组件解释。看不到渲染结果就记录未检查，不将源码块计为完成图。图件与正文同时维护，保密资料不送入外部工具。
>
> 可维护性从具体维护任务反推诊断、隔离、变更和恢复能力，说明操作位置与权限；可测试性从关键保证反推控制、观测、独立判定、清理，以及环境、数据构造、快速复位、并行隔离和自动化复现。硬件制造测试要反推可检测缺陷和产品预留。有副作用的重新执行或在途资源复用须先核对权威状态，满足旧执行者停止/隔离及幂等/去重条件；真正只读且无跨请求资源预留的重新采样须有新观测上下文、目标核对和有界清理，不当作原操作恢复。不得擅自新增机制。
>
...
用户的明确要求，不能从指南或 README 更新推导项目已升级；本轮只调整系统模板及配套指导，
