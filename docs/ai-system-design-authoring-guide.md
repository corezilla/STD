...
版本：0.3.1-draft.11 · 日期：2026-09-10 · 状态：编写方法草案，待完整写作任务验证
...
本版模板与图件核对基线：STD commit `4737d5427c42abba8e4ded4e5f44d99f981cf4b9`；
该提交包含待发布系统模板 `6.4.0` 的短封面、章节布局、场景／架构样板、软件分层、自研 FPGA
程序及板卡布局和逐组件职责说明。本文新增的绘图操作与复审修订由本指南版本及所在提交记录，
不倒称已包含在核对基线中。下表为本版对应关系：
...
| 系统架构／§5.1 | [轻量系统图](assets/system-design-authoring/system-architecture-light.png)；需纯矢量时参考[简洁框图源](assets/system-design-authoring/system-architecture.svg) | 主要设备或服务、系统边界、层次和少量主要连接；不下钻缓存、DMA 等内部模块 |
| 软件架构／§8.1 | [software-layered-architecture.svg](../templates/diagrams/software-layered-architecture.svg) | 左侧层名、右侧纯文字模块、浅层底色与较深模块色；无图标、无连接线 |
| 自研 FPGA 程序架构／§9.1 | [fpga-program-architecture.svg](../templates/diagrams/fpga-program-architecture.svg) | 自研逻辑分组、接口边界及必要的主数据方向，不画成 FPGA 芯片资源图 |
| 自研板卡顶层布局／§7.1 | [board-top-layout.svg](../templates/diagrams/board-top-layout.svg) | 板形示意、器件与功能区、主要接口位置和联系；不画成 RTL 模块图或实际 PCB 布线 |
...
这些相对链接用于当前 checkout 导航。模板、章节和图件以页首的核对基线为准；历史来源提交
不包含后续改进，指南修订则以本文版本及所在 Git 提交为准。实际项目仍使用自身已采用且获准
的来源，不因核对本指南就自动对齐该基线。后续采用、模板或 STD 版本升级遵循项目与用户的
明确要求，不能从指南迁入或 README 更新推导项目已升级；本次不更改模板版本、项目 lock，
也不发布新的 STD 版本或 RAG 清单。
