...
### 8.1 跨 Owner、软件—FPGA—板卡承接变体

本例按机制模板 §14 的四段式要求侧分解，但不把所有对象强称为“软件模块”：

| §14 映射 | 教学承接结果 |
|---|---|
| 参与方 → 架构对象 | Host 应用 → `M-XFER-CLIENT` 软件模块；驱动 → `M-XFER-DRIVER` 软件模块；FPGA 执行方 → `F-XFER-TOP` FPGA 程序；平台隔离/排空 → `B-XFER` 板卡及平台能力。Owner 可分别属于应用、内核/驱动、FPGA 和硬件团队。 |
| 步骤 → 责任 | 驱动主责冻结输入、发布描述符和软件可见状态；FPGA 主责执行和完成记录；板卡/平台主责旧访问隔离与排空证明。任何单方 ACK 都不能替代 X-R3 的组合条件。 |
| 跨对象接口 | 软件逻辑入口引用 `IF-XFER#SUBMIT/QUERY/CANCEL/RELEASE`；驱动—FPGA 交接引用 `IF-XFER#DESC/COMPLETE` 及实际项目补齐的通知、停止和 fence 绑定。这里仅说明交接语义，不复制 §3/§4 的字段和签名。 |
| 下级设计输入 | 驱动设计深化映射、同步和状态机；FPGA 顶层设计深化描述符消费、停止确认与迟到完成隔离；板卡设计说明地址域、复位域及隔离/排空能力；组合测试以 X-V1–X-V4 验收。 |

项目采用时，每个对象在自己的专项设计中反向登记这些输入及本地/组合验证。共同协议未定、板卡能力
不存在或 Owner 冲突时，机制保持设计缺口；不能让某个下级对象自行选择一套局部协议后宣称组合闭合。
