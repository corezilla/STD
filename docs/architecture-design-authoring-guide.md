...
第 3 章系统概览直接容纳 §3.1 系统架构、§3.2 组成与职责、§3.3 物理与逻辑对应关系，
不再另立“总体结构”章。开篇承接产品问题和环境，以前 1–2 屏建立总体认识，再逐组件说明
分工、约束和部署落点；不以页数强行压缩职责与预算。核心处理和主要取舍要能独立读懂，
完整业务过程统一在 §5.2 展开，不能两处维护同一套流程。图或表用于说明关系，不能取代解释。
...
**正确写法**：“本服务不拥有板卡设计，§6 按已批准 TAIL-003 省略；节点资源和故障域在 §7.6
描述。服务存在管理入口和用户输入，因此 §14 适用，需说明信任边界和授权位置。”
...
每张架构图应在图内或紧邻图注显示：图号和标题、视角、视图状态、Baseline ID、适用版本/配置/拓扑、
对象和 Owner、责任/信任边界及符号含义。软件分层组成图以左侧层名和右侧纯文字模块分组表达
包含关系，不要求图标或连接线；浅层底色与较深模块色块辅助辨识，正文解释分层依据和职责。
FPGA 程序图展示项目设计的逻辑模块，不以芯片资源图代替；板卡顶层布局展示自研板卡的器件、
功能区和接口位置，不等于实际 PCB 布线。两者保持层次对应：板卡中的 FPGA 器件在程序图里
展开，外部存储芯片与程序内队列/控制器分别命名。已确认样板见系统模板 §6.1、§8.1。
软件分层组成图的调用和数据流另图表达；交互图需注明箭头方向、数据或控制含义及协议/IF ID，并在图旁解释
...
**正确写法**：“FIG-3-1，Current，Baseline B-01，单节点配置。接收单元（Owner A）通过 IF-01
把校验后的事件提交到处理单元（Owner B）；处理单元通过 IF-02 写入结果存储。实线为业务数据，
点线为管理控制；存储位于外部信任域。”图旁说明写入失败在哪个边界返回，真实协议由接口条目给出。

**完成检查**：把图和紧邻图注单独交给读者，读者能识别对象、层次/边界、Owner、状态和适用范围；
交互图的方向与交接语义清楚，组成图没有被迫塞入调用连线。
...
模型不仅是给数字贴标签。按模板 §12.1，从单次输入的资源需求推到整条路径：相同资源上的
...
系统设计保留“为什么这样分工、如何协作、全局约束和失败影响”，并与正式机制/契约一起确定
全部公共操作、字段、错误、命令和适用硬件连接，系统 §10 作为完整呈现或精确关联入口。
内部算法、私有布局、安装步骤和测试程序由下级维护。需要向下引用时，正文仍解释决定性语义；引用
...
细节有下级文档，就删除读懂主路径必需的解释。§13 给出验证策略、oracle 和覆盖；具体测试步骤
引用测试规格/程序。

必要的数据格式图与字段摘要可以从固定版本的机器定义提取或生成，标出来源、摘要范围和核对
方法，并明确它是解释性视图，不是另一份可独立修改的规范。还没有机器定义的新设计可以写
完整的 Proposed 结构并记录责任；未定共同字段是设计缺口，不能只登记后便称可交接。
需要完整展示时从唯一来源生成/核对，不手工维护竞争副本；不能虚构现有 Schema，或以
“没有 Schema”为由不解释数据。逻辑结构图必须与真实字节布局区分。

**完成检查**：每个跨 Owner 边界只有一个行为/字段 authority；正文能够解释其协作责任，同时
与所引用版本一致。实际条款已读并核对，提供与消费的目标、字段、状态和错误含义一致，或
有明确定义且可验证的转换关系；只
检查路径存在不算契约检查。可读性、完整性与去重必须同时满足。
...
系统模板 §3.2 还必须把各章决定分配给下游，机制模板 §3.1、通用单元模板 §1.1 与硬件/FPGA
...
或“升级手册另述”。公共参数全集由唯一契约在设计阶段确定，下级补内部实现，操作手册
承接逐条执行步骤，但不重新决定
...
模板第 5 章是“重要过程”：§5.1 建立过程索引与工作模式，§5.2 先走通一次完整业务处理，
§5.3 写启动，§5.4 写配置加载与生效，§5.5 写停止/重启，§5.6 写异常恢复，§5.7
写模式切换。新增的生命周期说明都要有段落式推导、流程图或时序图及步骤，不能用“各模块
初始化完成”跳过系统顺序。纯软件也需要启动与初始配置说明，但不需要复制不适用的上电步骤。
这是阅读顺序而非执行顺序，业务仍必须满足就绪与配置前提。§17.2 将系统结论转成下级详细
设计与验收任务，并关联风险、技术债、未决项和预设计，不能因调整目录删除这些内容。
...
系统级过程由第 5 章串起；详细机制按本指南 §6 分工，升级等已有完整专节的过程在模板 §5.1
索引其唯一正文，不再复制一份。
...
这些缺口表明本例尚处预设计，不能直接将公共接口标为可交接；须先补齐共同契约，再分配双方实现。
...
§13 用六节讲清主要测试方法与判定、测试控制与故障恢复、环境快速部署与复位、并发环境隔离、
自动化复现以及覆盖验收。每节围绕实际场景简要解释做法与限制，不堆砌测试术语。可观测性和
统计、日志与故障定位要求写在 §11.2，自检与诊断作为其下的 §11.2.1；§11.3 专门说明升级与回滚。
指示灯、状态查询、维护/调试命令，以及替代依赖、旁路和环回接口的能力要求写在 §10.4 对应的逐接口记录。
§13 引用这些设计来组织验证，不重复定义机制。
...
公共参数、互连引脚和日志字段须完整定义或精确关联；解析实现和内部存储可由下级决定。
系统保留关键语义与分工，不退化成只有需求编号的索引。

自检在 §11.2.1 继续分为检查项目、依赖与真实路径覆盖、结果汇总和退出恢复；指标日志及
跨组件关联在 §11.2.2/§11.2.3 定义单位、窗口、时钟域、复位代次和部分观测。
§13.2 的控制点说明启用、匹配、命中与撤销确认，以及真实/模拟支持范围和中断清理。
这些是通用设计问题，单组件或不具备相应能力的产品按实际边界裁剪，不机械新增系统能力。

从能力子项检查覆盖，再分别推演不同协作和清理机制。明确运行时统筹与状态 authority，
不要将所有维护、诊断和测试合并成一个宽泛任务。未决共同方案先预设计，已有局部契约先
读取并纳入组合，旧条款冲突先修订，设计完整后才交下级实现；只影响相应依赖范围。
一个完整能力包的虚构写法见 [AI 指南 §8.8](ai-system-design-authoring-guide.md#88-完整小例两个单元的只读版本核对)，
其只读协议不适用于有副作用的动作；本指南后续目录导入示例仍是带未决项的预设计示例。
...
宣称整体通过。系统模板 §3.2 的虚构算例把它分为 16 + 24 + 16 + 8 MiB，仍需明确公共项、
...
系统模板 §3.4 先确定机制 ID、Document ID、目标文件名及范围/依赖，后续逐份编写时保持同一映射。
Planned 用代码路径、已成文版本 none；实际成文后才链接并绑定版本/锚点。系统正文依旧解释原理和代表失败，
不能以机制清单替代系统设计。§9/10 的类型与操作沿[统一映射规范](interface-data-mapping-standard.md)
关联目录、机器源、行为与下游；完整例见[虚构暂存导出](examples/interfaces/README.md)，不复制任何产品协议。
