...
[软件设计对象编码规范](software-object-identifiers.md)。编号表达对象归属，不增加设计层级。

当前采用以下设计分工，不采用递归软件子系统。名称表示设计责任，不是目录深度：
...
总体系统设计确定三条分支的职责、跨领域交互、公共机制和预算；软件系统设计确定软件整体方案、
直属软件子系统及按需直属模块的协作。软件子系统设计展开内部模块组织和运行方案，软件模块设计
完成模块内部实现。不得把软件系统改称一级子系统，或把软件子系统再拆成二级子系统。
固件系统设计描述自研 FPGA 程序，不是 FPGA 芯片本身的资源架构；多 FPGA 时可先分到各程序顶层，
再落实 RTL 功能模块。硬件系统设计确定板卡间连接与分配，板卡设计展开器件、连接和布局，电路
功能单元设计落实局部电路。两条链均不套用软件子系统模板；单 FPGA/单板项目可合并承载相应层次，
但需明确范围和信息项映射，不为层级重复建文。

`parent_document_id` 指向同项目内直属父对象的设计 Document ID，不是依赖、目录或模板继承。
正文记录对象 ID、设计角色及父对象，与父设计组成表和 metadata 一致。`design_level` 是通用粒度：
总体和领域系统为 `system`，软件子系统为 `subsystem`，模块为 `module`；板卡及电路单元按专项实际
对象粒度记录。角色由正文明确，不新增 subsystem-2，也不计算或保存子系统深度。
父设计只确定直属组成、跨边界协作及共同保证；必要跨层约束标明来源、目标、理由和下级自由度。

新稿可用 `new-design --parent-document-id <父Document-ID>` 建立父链；
`validate-design <完整设计输入目录> --check-design-hierarchy --json` 核对父引用、项目、环及子系统类型，
拒绝子系统继续挂在子系统之下，返回父文档关联而非深度。输入需包含所有父设计；该检查不从
`system` 枚举推断软件领域，也不证明正文归属或业务正确性，三条领域链仍需对照组成表复审。
不加开关保持旧文档校验行为；已采用版本的项目不自动补链或升级。

| 设计对象/要交付的决定 | 模板入口 | 内容与上层引用 |
|---|---|---|
| 总体系统设计 | `design.system`（原“系统设计”，ID/路径不变） | 完整产品全景、三条分支、端到端过程及系统级约束；纯软件产品也可有总体设计 |
| 软件系统设计 | `design.software-system`，`design_level=system` | 软件整体概要、直属子系统及直属模块协作；纯软件项目直接作顶层，领域模式承接总体设计 |
| 软件子系统设计 | `design.subsystem`，`design_level=subsystem` | 承接软件系统设计，完成模块概要、运行与业务设计，不递归子系统 |
| 软件模块设计 | `design.definition`，`design_level=module` | 承接软件系统或软件子系统，在分配边界内完成内部实现与验证 |
| 总体系统机制设计 | `design.system-mechanism`（ID/路径不变） | 横向细化总体系统的公共机制，不是软件/固件/硬件分解链上的固定一级 |
| 固件系统/FPGA 程序顶层/RTL 功能模块 | 现有 `design.fpga` 专项按实际范围选用；领域系统完整概要覆盖需裁剪核对 | 明确 FPGA 程序归属、顶层到 RTL 的约束及本地/组合验证，不写芯片内部资源说明来代替程序设计 |
| 硬件系统/板卡/电路功能单元 | 现有 `design.hardware` 专项按实际范围选用；领域系统完整概要覆盖需裁剪核对 | 明确板卡连接、板内设计及局部电路承接，不重复软件子系统文档 |
| 数据字段阅读视图 | `design.data-dictionary` | 逐字段解释语义、ownership 与使用；类型/编码以固定 Schema/IDL 等机器源为唯一 authority，引用所属对象及契约 |

软件系统模板同时支持两种位置：纯软件项目顶层不强制另建重复的总体系统稿，parent_document_id 可为空；
总体系统的软件领域设计则记录真实父 Document ID 和固定分配。模式在正文说明，不新增 metadata 枚举。
软件模板不拥有板卡/FPGA 内部设计，依赖写在环境、部署和接口中；不得把子系统模板上推冒充软件整体。
既有软件系统文档按已采用模板继续有效，升级由用户明确提出后再对齐。

`design.system` 的写作 profile 为 `software-system`、`integrated-system`、`hardware-fpga`。
这里的 `software-system` 表示纯软件产品的总体系统写作适用性，不等于 `design.software-system` 的 Template ID；新建纯软件顶层优先用软件专用模板，旧稿不自动迁移。
...
总体系统模板 §3.4、软件系统模板 §3.5 分别维护本层机制清单和文档映射，允许预定 ID/文件名后逐份写作。
Planned 路径不是已存在链接或实现输入，成文后才绑定真实版本；详细规则见[映射规范 §5](interface-data-mapping-standard.md#5-先登记机制再逐份编写)。


- `design.system` 保存系统全景、软件/固件/硬件分工、全局策略和约束分配，并自包含地解释端到端原理、
  关键阶段、系统级约束和代表失败；机制目录是导航，不是正文的替代品。
- `design.software-system` 在 §3.1–3.2 展示软件架构及组成职责，§3.6 定义功能，§6 保留端到端过程，§8 关联接口。
  纯软件顶层的跨组件机制也使用 `design.system-mechanism`，不为此虚构总体系统父稿；领域模式优先
  复用总体已定义的共同机制，只将本软件范围内需独立细化的机制另立文档，不重复跨领域协议。
- `design.system-mechanism` 每份细化一个系统公共机制（纯软件项目也使用同一模板，可以跨软件、固件、硬件责任单元），唯一维护其详细状态转换、参与方协议、
  完整正常/失败步骤及恢复规则；公共字段与操作必须已完整定义，已有机器契约保持唯一来源。
  总体系统 §10 或软件系统 §8 统一呈现或精确关联；没有机器源时先用受控设计明确共同定义，不伪造引用。
- `design.subsystem` 在 §1 承接功能与约束、§2–3 完成第0层架构及第1层模块展开、§4 设计运行组织、§12/§14 组织验证与下游交接；它引用公共机制，
...
机制模板为 16 个主章，§4 数据结构、§5 接口分开，§14 把机制要求分解到责任单元及接口；既有信息保留，不复制完整产品系统的全部专业章节。条件信息项和纯软件 API 裁剪示例见机制模板附录 A。
开篇用途图、协作/时序/数据/状态/异常/测试六幅关系图与两幅有副作用过程/条件依赖图见
[机制图形样板](../templates/diagrams/mechanism/README.md)，按表达需要选择，不设置图片数量 Gate。
逐能力写作、图例复用和下游承接方法见[总体系统机制设计 AI 编写指南](ai-guides/system-mechanism.md)，
...
系统约束分配在总体系统模板 §3.2 或软件系统模板 §3.4 汇总，机制模板 §3.1、通用单元模板 §1.1，以及硬件/FPGA 专项模板 §1.1 分别记录承接与落实；专项模板 §12 把 Constraint ID 接到本地与系统组合验收。
