...
**数据与错误承接**：先列本契约拥有/引用的全部 Data/Type ID、request/response/event/error 角色及固定机器源，再逐字段定义类型、可空性、范围/单位、条件有效性与合法/拒绝实例。上级公共类型与错误码只引用原 ID、版本和 selector。
...
**错误承接**：逐个错误列系统 Error ID 或明确的契约私有码、触发事实、判定者、载荷、操作状态/副作用、调用方动作和兼容处理；公共错误码与系统目录和机器源核对，不自行发明代码值。
