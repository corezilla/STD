...
> 本模板名称为“总体系统设计”，沿用 Template ID `design.system` 与现有文件路径。
> 总体系统设计统筹软件系统、固件系统（自研 FPGA 程序）和硬件系统；领域分解及模板选择见所采用 STD 的 `docs/template-selection.md`。
> 总体系统机制设计是横向公共协作的详细设计，先在 §3.4 建立机制及文档映射，不作为分解树中的固定一级。
...
- 本模板描述总体系统级设计对象；下分软件系统、固件系统和硬件系统。软件系统下的软件子系统
  采用 `design.subsystem`，软件模块采用 `design.definition`；板卡/FPGA 按领域专项承接，不重复建文。确需跨层时明确
  系统级主体、下钻目的和截止深度，`cross-level` 不能成为复制所有详细设计的理由。
- 多个子系统必须共同遵守的行为、公共对象和接口，须在系统设计及其正式机制/契约中确定，
  不能以“下级细化”推迟共同方案。下级自行决定不改变外部保证的内部算法、RTL 实现和局部布局。
- 已有权威定义按固定版本复用；§10 集中呈现或精确关联完整公共契约，可从机器源生成展示表并
  校验一致性，不手工维护第二份真相。引用不存在或内容未定的契约仍是设计缺口，不算设计完成。
- “未来可能支持”必须与本版本承诺分开。
...
从 §9.9/§12.1 等推导，分清单实例和整机、独占和共享、常驻与峰值、并存副本、公共开销与
...
**本节目的**：让不同组成部分能一致地指认操作对象，而不是各用一个模糊的“设备编号”。

**必须写清楚**：对象层级、标识分配者与解析者、唯一性范围、生命周期，以及替换、重启或
重新部署后的身份变化。单实例系统可用固定映射，不为填写本节新增分布式身份服务。

**编写规范**：选择一次定位、查询或控制操作，沿用户可见名称到实际执行对象逐级解析，说明
哪些是稳定身份、物理位置、运行实例或本次操作编号。地址不自动等于身份；对象替换后，旧请求
能否误命中新对象必须有判据。复用项目已有定义，公共字段在 §9.2 唯一维护；拓扑变化的
接受、拒绝与刷新责任由过程设计承接，不能仅画一张静态关系图。

**抽象示例**：虚构采集设备把“槽位 2”作为位置，把控制器读取的序列号作为板卡身份；更换
板卡后针对旧序列号的维护请求被拒绝。槽位相同不表示对象相同，不允许自动改目标后重试。

**完成条件**：给定一个外部目标，可以唯一解析到本次运行对象；替换或重启后，能判断旧操作
是否仍合法。字段范围和生命周期有定义，不能只靠名称相似关联。

</details>

| 对象层级 | 标识及唯一性范围 | 分配/解析责任 | 生命周期与替换规则 | 拓扑来源/公共契约 |
|---|---|---|---|---|
...
**本节目的**：明确业务路径失效后还能从哪里查询、诊断和恢复，以及哪些访问路径会一起失效。

**必须写清楚**：业务、管理、调试路径的入口与逐跳依赖、权限、故障下的可达性及受影响功能。
没有独立带外通道时直说，不因本节强制增加专用硬件或守护进程。

**编写规范**：沿 §3.3 的载体图分别走一次正常访问和共享部件失效后的访问。区分“业务停止
但控制可达”与“同一链路上全部不可达”，指出统计和恢复命令是否依赖已失效部件。明确失联时
能得出什么、不能得出什么；需要现场复位时说明保留条件和责任，不把超时等同设备已停止。

**抽象示例**：虚构设备的查询和采集共用主机链路，链路断开后两者都不可用；机箱指示灯仅能
反映本地供电，不能证明采集任务已安全结束。软件保留结果未知，现场恢复按停机流程进行。

**完成条件**：每类关键故障能追到仍可用的入口或明确的不可达边界；不存在依赖失效通道完成
恢复的循环，也不从不可达推断数据或执行状态。
...
按实际交付范围核对业务、配置、管理、诊断、升级、安全、测试支撑及适用的功热保护能力。
把“支持维护”分解到能够分别设计和验收的操作或保证；不同目标、权限、路径或失败影响需要
分别说明，不能合并成一行后用一个代表成功案例覆盖全部。等价范围可以合并，但须说明依据和
例外，不机械枚举所有组合。每项能力关联参与方、过程、公共契约和系统验收位置；尚缺设计与
仅待实现分开。产品不需要的类别写依据，不因本清单增加产品能力。
...
公共字段与行为必须已在 §10 或其精确关联的唯一契约中完整定义，内部实现由下级规格负责。
验收条件应从上述输入、判定和结果自然导出，不另写
...
**本节目的**：把跨组件能力变成双方都能实现的共同方案，而不只分配文档 Owner。

**必须写清楚**：操作入口、运行时统筹者、参与方、权威状态保存者、生效点、结果汇总者、
对象/操作身份及并发规则。只涉及单组件时说明本地完成边界；不强制引入新的中央控制器。

**编写规范**：按 §4.1 的实际能力识别协作，不只检查主业务。复用现有调用链，逐项写谁
决定、谁执行、谁确认；Document Owner 不等于运行时统筹者。指出状态保存在哪里、进程退出
后保留什么，若无持久记录就限定可承诺的恢复范围。身份、对象、结果和状态必须与 §10
共享契约对应；仅“管理模块协调各方”不能作为设计结论。可复用同一通用操作协议，但每项
能力仍要说明适用目标、参与方和特殊失败差异。

**抽象示例**：虚构双采集单元的静态配置只在启动时生效；已有启动程序逐个加载并核对版本，
两者均确认后才开放业务。它不是在线事务服务；任一失败就保持业务关闭，诊断入口仍可访问。

**完成条件**：下游知道实际收到什么、谁能改变权威状态、何时算整体完成；不必自行决定
系统级政策，也不会因缺少统筹方而各自实现一套状态机。

</details>

| Process/能力 ID | 入口/运行时统筹者 | 参与方及动作 | 权威状态/保存与恢复 | 对象/操作身份 | 生效及总体完成判据 | 公共契约 |
|---|---|---|---|---|---|---|
...
**本节目的**：证明协作在响应丢失、部分完成或统筹方退出后仍有可解释的系统结果。

**必须写清楚**：每一步的输入、状态变化、确认所证明的事实，整体与逐目标结果，超时、重复、
取消、参与方/统筹方退出后的核对与清理，资源释放和业务重新开放条件。

**编写规范**：按操作顺序推演“请求未到达”“动作完成但响应丢失”“仅部分目标完成”和
“统筹进程退出”。将已受理、已生效、已清理分开；超时先是观察不到结果，不是回滚成功。
有副作用的重新执行或在途资源复用，只有权威状态可核对且旧执行权限已停止或隔离、满足
幂等/去重条件时才允许；无此保证就保持阻塞或显式转人工。真正无业务副作用、无跨请求资源
预留的只读查询可重新采样，但须核对目标身份、使用新的观测上下文，并限定旧查询的清理
期限；新采样不是原操作恢复，也不能证明旧执行已停止。不默认增加 lease 或自动接管机制。详细转换可由机制文档
唯一维护，但本节必须保留端到端原理、关键确认、系统限制及代表失败。

**抽象示例**：虚构只读诊断中，一个目标返回、另一个超时，汇总结果写 Partial，并分别标明
采集时刻。不能返回“全机正常”，也不能把第一个结果当作第二个结果。恢复查询须保持目标身份。

**完成条件**：从任一关键中断点能说明已知/未知状态、合法下一动作、残留资源和重新开放
条件。发送方与接收方对同一个确认含义一致，失败路径有系统验收落点。

</details>

| 阶段/参与方 | 输入/前置状态 | 动作及权威状态变化 | 确认语义 | 超时/重复/退出后的核对与处置 | 资源与业务状态 | 验收 |
|---|---|---|---|---|---|---|
...
| 顺序 | 输入单元 I | 处理单元 P | 调度单元 D | 输出单元 O |
|---|---|---|---|---|
| 1 | 原始数据、接口元数据 → | 接收并校验 |  |  |
| 2 |  | 解析、分类、处理状态 |  |  |
| 3 |  | 结果数据、转发描述符 → | 接收并选择目标 |  |
| 4 |  |  | 按目标与优先级调度 → | 接收结果 |

> FIG-5-1 主路径顺序示例，非项目事实。视图 Current；EXAMPLE-B-01，示例 E1/单节点配置。
> I/P/D/O 的 Owner 分别对应 §3.2 中的责任条目；步骤与数据类型见表中的箭头，跨界协议由
> IF-01/IF-03/IF-04 定义。本示例全部为当前路径、Implemented/NOT_RUN。
...
什么。机制、字段及一致性范围引用 §7.3/§10.1，本节将各参与者串成一次完整生效过程。
...
配置项的作用域/生效规则摘要和一条完整变更时序；完整字段由配置契约维护，控制入口引用 §10.1。
...
本章按数据性质选择适用类别，不按输入/输出重复定义同一类型。每个真实结构以名称作带编号的小节标题，并在同一处完成声明、逐字段或逐值定义、约束、来源、所有权与寿命、合法与拒绝实例及验证；不另设数据结构清单或公共编码章节。下方教学示例直接写在模板各类别内，供作者仿照；生成项目文档时删除示例，只保留实际设计。

**本章目的**：描述系统内部真正流动和持久化的数据，以及各阶段如何改变它。

**必须写清楚**：主要数据流、描述符/元数据、状态表、缓存与存储、字段 ownership、生命周期和容量依据。

**公共错误类型**：§9.1 先定义错误码及错误载荷结构，逐码说明含义、触发事实、结果、下级用法和载荷约束。不能仅列错误名称或推迟到模块自行决定。
...
**本节目的**：先定义系统公共错误类型，再统一本层拥有的其他共享类型与枚举语义，使下级可直接承接。

**必须写清楚**：公共错误码逐码给出唯一含义、触发事实、结果与副作用、调用方动作及下级承接；对应错误载荷在同节定义字段、关联身份及条件约束。其他类型与枚举逐值说明含义、合法范围和未知值处理。逐结构记录 Data/Type ID、固定来源、字段、所有权与寿命、合法和拒绝实例及验证。

**编写规范**：复用上级类型时仅给固定引用和本层投影，不重定义值。本类别不适用时在章首说明原因和 tailoring 依据，不保留空小节。如果字段已有机器定义，应固定版本并逐项核对；教学例子不得成为项目默认值。

**抽象示例**：`QUEUE_FULL` 表示本次请求未受理，其错误载荷不应包含已创建的任务 ID；任务状态 `RUNNING` 只表示执行已开始，不代表成功完成。示例不是项目的机器契约。

**完成条件**：各使用处对同一值作一致解释，且没有与其他章节重复维护同一字段定义。

</details>

<!-- 编写建议：首先定义公共错误码及对应错误载荷，再写其他公共基础类型与枚举。已由上级或机器源定义的值只引用固定来源。错误码逐值写含义、触发事实、结果、副作用、调用方动作及下级使用；载荷逐字段写关联身份、条件约束和敏感信息边界。其他枚举逐值写含义、范围、未知值处理和适用边界。 -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`InspectionErrorCode`（公共错误码枚举，虚构）**

```text
enum InspectionErrorCode { QUEUE_FULL }
```

- **Data/Type/Error ID、来源**：枚举 `DATA-EX-ERROR-CODE`，其中 `QUEUE_FULL` 为 `ERR-EX-QUEUE-FULL`；示例机器源为 `Proposed: InspectionErrorCode`。正式项目须固定实际 Error enum/Schema 的代码值、版本和核对结果，不把教学名称当作已分配码。
- **`QUEUE_FULL` 的含义与边界**：受理队列达到确定上限，本次请求未受理且未创建任务；不表示已受理任务执行失败，也不表示结果未知。其他未知码不得擅自映射为 `QUEUE_FULL`。
- **触发、结果与副作用**：受理责任单元在入队前依据权威队列容量事实判定；返回此码时不得有新任务或已占用的执行资源。调用方可在接口规定的期限内按同一请求身份重试，不能把重试理解为已有任务恢复。
- **下级承接与载荷**：受理模块产生此码，边界适配模块可透传，界面可据此提示稍后重试；错误响应使用下方同节定义的 `InspectionError`，其中 `task_id` 必须为空。
- **合法/拒绝实例与验证**：队列满且无新任务时返回 `QUEUE_FULL` 合法；任务已创建却返回此码必须拒绝。`V-EX-QUEUE-01` 核对触发事实、错误码和无副作用，教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->

<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`InspectionError`（错误载荷结构，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-ERROR`；表示一次请求的错误响应。示例机器源为 `Proposed: InspectionError`；正式设计须固定错误载荷 Schema 及其版本。
- **`code`**：必填，取本节 `InspectionErrorCode` 的已定义值；本结构不重新分配错误码。
- **`request_id`**：必填、非空字符串，关联失败请求。
- **`task_id`**：可空字符串；当 `code=QUEUE_FULL` 时必须为空，因该码定义为“未受理”。其他错误码是否要求该字段，由各码的条件约束决定。
- **约束与生命周期**：错误响应由边界适配单元构造，并随本次请求返回；日志可保留关联身份，但敏感信息不得未经授权复制到载荷。实际编码与长度由 Schema 固定。
- **合法/拒绝实例**：`{code:QUEUE_FULL, request_id:"req-7", task_id:null}` 在确实未受理时合法；任务已创建却返回 `QUEUE_FULL` 属语义错误，必须阻断。
- **验证**：`V-EX-ERROR-01` 检查字段条件、关联身份和敏感信息边界；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
- **错误载荷**
  ...
  <本节错误载荷结构的类型 ID、必填字段、关联身份和敏感信息边界>
- **调用方动作**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`InspectionState`（其他公共枚举，虚构）**

```text
enum InspectionState { ACCEPTED, RUNNING, SUCCEEDED, FAILED }
```

- **Data/Type ID、含义与来源**：`DATA-EX-STATE`；一次检测任务的权威业务状态。示例机器源为 `Proposed: InspectionState`；正式设计须给出实际 Schema/IDL 路径、版本和核对结果，或记录形成契约的 Owner 与 Gate。
- **逐值定义**：`ACCEPTED`＝已受理但尚未执行；`RUNNING`＝已开始执行但无最终结果；`SUCCEEDED`＝结果已发布；`FAILED`＝执行终止且未发布成功结果。未知值拒绝，不映射为 `FAILED`。
- **约束与生命周期**：状态键为 `task_id`；只有任务状态的权威持有者可改变值，发布后读者才可观察。`RUNNING → SUCCEEDED` 必须已有可定位的结果；终态不得再回到 `RUNNING`。任务保留期限由系统级策略决定，此例不虚构期限。
- **合法/拒绝实例**：已发布结果 `result_id=res-7` 后置 `SUCCEEDED` 合法；只有执行开始记录、没有结果时置 `SUCCEEDED` 必须拒绝。
- **验证**：设计向量 `V-EX-STATE-01` 检查终态前提和未知值拒绝；教学示例 `NOT_RUN`，不表示验证通过。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**本节目的**：定义业务操作实际交换和保存的对象，以及跨步骤保留的运行状态，使该类数据可由下级直接承接。

**必须写清楚**：逐字段类型、必填性、范围、键及跨字段条件；运行状态还要说明唯一写者、转移前提、可见点与恢复事实。逐结构记录 Data/Type ID、固定来源、字段、所有权与寿命、合法和拒绝实例及验证。

**编写规范**：同一结构无论作为输入、输出还是运行状态只定义一次。状态名称不代替进入条件，内存态与持久态分开。本类别不适用时在章首说明原因和 tailoring 依据，不保留空小节。如果字段已有机器定义，应固定版本并逐项核对；教学例子不得成为项目默认值。

**抽象示例**：InspectionRequest 用非空 request_id 关联一次检测操作；InspectionRuntimeState 进入 RUNNING 需有已受理执行的事实。示例只说明写法，不是项目的机器契约。

**完成条件**：合法与拒绝实例均能依字段和规则判定；正常、失败和重启路径能定位有效状态，且没有与其他章节重复维护同一字段定义。

</details>

<!-- 编写建议：按真实业务或运行状态对象逐字段说明类型、必填/可空、范围和跨字段不变量；同一结构在多个接口使用时只定义一次。运行状态还要说明唯一写者、允许转移、可见点和恢复事实。虚构例子：InspectionRequest 的 request_id 非空且标识一次业务操作。 -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`InspectionRequest`（业务对象，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-REQUEST`；代表一次待受理的检测请求，示例机器源为 `Proposed: InspectionRequest`。正式项目需绑定实际类型定义和固定版本。
- **`request_id`**：必填、非空字符串，作为同一业务请求的稳定身份；实际编码及长度上限由 Schema 确定。
- **`image_uri`**：必填、非空字符串，指向待检图像而非图像字节副本；URI 方案和访问权限由图像存储契约确定。
- **`policy_version`**：必填、正整数，指定本次请求采用的已发布规则版本。
- **跨字段与寿命**：同一 `request_id` 重放时，`image_uri` 与 `policy_version` 必须保持一致；冲突值不得创建第二任务。受理方创建任务记录后保留请求身份；原图的所有权与释放期限另由图像存储契约决定。
- **合法/拒绝实例**：`{request_id:"req-7", image_uri:"asset://image/7", policy_version:3}` 在引用和版本均存在时合法；同一 `request_id` 携带另一个图像 URI 应拒绝并返回已定义的冲突错误。
- **验证**：`V-EX-REQUEST-01` 覆盖合法构造、缺字段和同 ID 冲突；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->

<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`InspectionRuntimeState`（运行状态，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-RUNTIME`；表示一项任务当前权威状态。示例机器源为 `Proposed: InspectionRuntimeState`；`InspectionState` 指向 §9.1 的同名教学类型，真实项目须改用自己的唯一来源。
- **`task_id`**：必填、非空字符串，作为任务状态键。
- **`generation`**：必填、正整数，标识该任务状态的更新代次。
- **`state`**：必填，取 §9.1 `InspectionState` 的已定义值。
- **`result_uri`**：可空字符串，仅在 `SUCCEEDED` 时必填，其他状态必须为空；URI 方案由结果存储契约确定。
- **约束与生命周期**：任务状态持有者是唯一写者，更新以 `(task_id, generation)` 防止旧完成覆盖新状态。状态读者可见点是权威更新提交后；进程内副本可失效，但不得将其直接当作持久事实。保留与恢复规则由系统策略明确。
- **合法/拒绝实例**：`{task_id:"t-7", generation:2, state:SUCCEEDED, result_uri:"result://7"}` 在结果已发布时合法；`state:SUCCEEDED, result_uri:null` 必须拒绝。
- **验证**：`V-EX-RUNTIME-01` 覆盖终态不变量和旧代次更新拒绝；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**抽象示例**：`config/inspection-policy.yaml` 在进程启动时整份校验并加载；编辑磁盘文件不等于运行实例已采用新配置。示例只说明写法，不是项目的配置路径或契约。

**完成条件**：能判定任一操作使用哪一版规则，且没有与其他章节重复维护同一字段定义。

</details>

<!-- 编写建议：以实际配置文件或配置源为单位展示完整格式，逐字段说明类型、范围、缺失/未知键处理，再写读取位置、优先级、生效点与失败出口；不要只给抽象对象，也不能由下级自创 fallback。 -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`config/inspection-policy.yaml`（完整配置文件，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-POLICY-FILE`；这是一份决定检测任务是否准入及其判定阈值的 YAML 文件，不是单独的内存对象。示例路径和内容均为虚构；真实项目须指定受控文件路径、唯一配置 Schema、版本及部署责任方。
- **`schema_version`**：必填整数，本例只接受 `1`；用于选择文件格式，未知版本拒绝加载。
- **`policy_version`**：必填正整数，标识本次加载的规则版本；发布后不复用。
- **`enabled`**：必填布尔值；`false` 表示不受理新的检测任务，不改变既有任务的终态。
- **`score_threshold`**：必填数值，闭区间 `[0, 1]`，单位为无量纲得分；`enabled=false` 时仍须通过格式校验，避免下次启用时读取无效值。
- **文件级约束与生效**：本例由进程在启动时读取并整份校验；缺字段、重复键、未知键、类型错误或越界值均使启动失败，不静默使用默认值。启动成功后进程固定一份已校验快照；编辑磁盘文件不会热更新，只有按启动过程重启并确认新版本后才生效。运行中任务的恢复规则由系统恢复设计决定。
- **合法/拒绝实例**：上面的完整文件在受控路径且通过 Schema 校验时合法；将 `score_threshold` 改为 `1.2` 或增加未知键 `fallback_mode` 时必须拒绝整份文件，不能部分采用。
- **验证**：`V-EX-POLICY-FILE-01` 覆盖完整加载、缺字段/未知键/越界拒绝、修改文件但未重启时版本不变；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**编写规范**：二进制报文先画字段布局图，标明位/字节位置、总长、端序和可变长边界，再逐字段解释；JSON 等非固定布局报文展示实际对象结构，不硬套位图。投递与重试动作在接口章节定义，结构只拥有载荷语义。本类别不适用时在章首说明原因和 tailoring 依据，不保留空小节。如果字段已有机器定义，应固定版本并逐项核对；教学例子不得成为项目默认值。
...
<!-- 编写建议：先按真实编码展示报文结构；固定二进制格式用位/字节布局图，JSON 等用对象结构图。随后逐字段说明宽度/类型、端序、关联身份、顺序、校验与兼容边界；投递/重试行为由对应接口定义。 -->
...
- **Data/Type ID、用途与来源**：`DATA-EX-FRAME-EVENT`；通知采集图像已可读取。示例机器源为 `Proposed: FrameReadyEvent`；正式项目须固定消息 Schema、版本和实际传输绑定，不从图推导 topic 或部署事实。
- **`magic`（bit 0–15）**：必填 `u16`，固定值 `0xA55A`；不匹配则按格式错误拒绝，不继续解析其他字段。
- **`version`（bit 16–23）**：必填 `u8`，本教学格式仅接受 `1`；未知版本拒绝，不猜测字段兼容。
- **`flags`（bit 24–31）**：必填 `u8`，本版必须为 `0`；非零保留位拒绝。
- **`source_id`（bit 32–63）**：必填 `u32`，非零，标识采集来源。
- **`frame_id`（bit 64–127）**：必填 `u64`，非零，标识该来源的一帧。
- **`sequence`（bit 128–191）**：必填 `u64`，非零，在同一来源内递增；不得据此跨来源排序，耗尽前须停止该来源的新报文而非回绕复用。
- **`image_ref_id`（bit 192–255）**：必填 `u64`，非零，引用该来源已发布且可读取的图像；引用的解析与授权由图像存储契约确定，报文本身不携带图像字节。
- **约束与寿命**：`(source_id, sequence)` 是去重键；同键不同 `frame_id` 或 `image_ref_id` 为冲突。报文只在图像可读取后发布；事件被消费不等于图像资源可删除。传输确认、重试和背压属于对应接口，不在此重复定义。
- **合法/拒绝实例**：`magic=0xA55A, version=1, flags=0, source_id=2, frame_id=7, sequence=42, image_ref_id=9` 按图编码为 32 byte 且引用已发布图像时合法；将 `flags` 改为 `1` 或同键改用另一个 `image_ref_id` 应拒绝，不覆盖原记录。
- **验证**：`V-EX-FRAME-01` 覆盖逐字段编码/解码、长度与端序、版本/保留位拒绝、重复与冲突报文；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**本节目的**：分别定义实际设备寄存器和 FPGA 数据表项的受控布局，使读写双方可按同一位域解释。

**必须写清楚**：先给寄存器地址空间总览，列出实际寄存器名称、偏移、宽度、访问属性、复位/初值和用途，再按名称逐个定义位域、读写副作用及错误行为；未列出的地址范围也要说明处理。表项则说明容量、索引、步长、字段布局、无效值、写入确认与生效条件。逐结构记录 Data/Type ID、固定来源、所有权与寿命、合法和拒绝实例及验证。

**编写规范**：寄存器地址总览用于寻址和覆盖检查，不是第二份位域定义；每个实际使用的寄存器仍须在唯一来源逐项展开。寄存器和表项分别成节，不把寄存器的访问副作用当成表项字段，也不只给一个位域图而省略地址与生命周期。图中明确逻辑 bit 编号与总线字节序的区别。纯软件项目无此对象时按 tailoring 删除，不虚构寄存器。本类别不适用时在章首说明原因和 tailoring 依据，不保留空小节。如果字段已有机器定义，应固定版本并逐项核对；教学例子不得成为项目默认值。

**抽象示例**：只读 `CaptureStatusRegister` 表达设备状态；`RouteTableEntry` 是索引表中的一条数据记录，两者有不同的地址和读写规则。示例只说明写法，不是项目的机器契约。

**完成条件**：寄存器读取和表项写入/读取都有唯一可核对的机器来源、字段解释及拒绝行为，且没有与其他章节重复维护同一位域定义。

</details>

<!-- 编写建议：只有真实拥有设备寄存器或 RTL 表项时才保留；寄存器先列地址空间总览，再以真实名称和固定机器源逐个定义。总览只列偏移、宽度、访问、初值和作用，不在两处维护位域。单寄存器写地址、位域、复位值与读写副作用；表项写索引、容量、步长、字段、无效值及生效点。纯软件范围不得为填模板虚构它们。 -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例一：`CaptureStatusRegister`（设备寄存器，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-CAPTURE-STATUS`；供控制单元读取采集状态。示例机器源为 `Proposed: CaptureStatusRegister`；正式设计须用固定版本 register map/RTL 逐位核对。
- **`reserved`（bit 31–3）**：只读，复位和正常读取均为零；读到非零视为格式/版本不匹配，不按已知字段解释。
- **`overflow`（bit 2）**：只读、复位值 `0`；发生输入溢出后锁存为 `1`，本教学例子中只在设备复位时清零。
- **`busy`（bit 1）**：只读、复位值 `0`；采集操作正在进行时为 `1`。
- **`ready`（bit 0）**：只读、复位值 `0`；初始化完成且可接收新操作时为 `1`。`ready` 与 `busy` 不得同时为 `1`。
- **访问与生命周期**：一次 32-bit 读取返回同一时刻的原子快照；写此只读地址返回总线拒绝，不改变状态。复位全零不等于已经就绪；何时从复位进入 `ready=1` 由设备启动过程定义。
- **合法/拒绝实例**：读取 `0x00000001` 表示就绪且不忙；`0x00000003` 同时声明就绪与忙，违反不变量，控制单元须报告设备状态异常，不能凭其中一个位继续执行。
- **验证**：`V-EX-REGISTER-01` 覆盖复位值、位域、只读拒写、原子读取和互斥状态；教学示例 `NOT_RUN`，不代表 RTL 仿真或上板结果。
<!-- STD_TEMPLATE_EXAMPLE_END -->

<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例二：`RouteTableEntry`（FPGA 数据表项，虚构）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-ROUTE`；表项给 FPGA 路由查找提供目标。示例布局为 `Proposed`；真实设计须以固定版本 RTL/register map 为机器权威，逐位核对容量、地址和生效规则。
- **`target`（bit 15–12）**：无符号 4 位，范围 0–15，选择输出目标。
- **`route_id`（bit 11–0）**：无符号 12 位，运行有效值 1–4095；`0` 表示空项，不参加有效路由查找。
- **索引、约束与生命周期**：表索引为 0–63，索引与 `route_id` 是不同概念；复位后 64 项全零。只有获授权的控制单元可在停止路由查找后以 16-bit 原子写入表项，硬件写入确认后，重新启动的查找才采用新值；在途查找不得混用旧新值。停止/确认/重启的调用条件由表项写入接口定义，本节只固定数据布局和生效边界。
- **合法/拒绝实例**：索引 `5` 写入 `target=2, route_id=7`，逻辑值为 `0x2007`、小端字节为 `07 20`，重启查找后有效；`target=2, route_id=0` 不可作为有效路由，读取时应按空项处理，不能把它当作已配置目标。
- **验证**：`V-EX-ROUTE-01` 覆盖地址步长、大小端编码、复位空项、无效值和写入后生效边界；教学示例 `NOT_RUN`，不代表 FPGA 仿真或上板结果。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**教学示例：`inspection_jobs`（数据库表，虚构；无数据库的系统不适用）**
...
- **Data/Type ID、用途与来源**：`DATA-EX-JOBS-TABLE`；保存任务的可恢复事实，示例 DDL 状态为 `Proposed`。真实系统须以受控迁移文件/Schema 的固定版本为机器权威，并说明数据库产品与约束支持情况。
- **`task_id`**：非空主键，对应任务身份。
- **`generation`**：正整数，用于防止旧更新覆盖。
- **`state`**：非空，采用 §9.1 枚举的持久编码；实际编码映射须固定。
- **`result_uri`**：可空，仅在成功终态非空。此例没有声称存在查询索引，真实查询路径应决定索引。
- **事务与生命周期**：唯一写入责任单元在承诺“任务已受理”前提交首条记录；状态与结果引用在同一事务中更新。崩溃后以已提交行恢复，不以页面缓存推断提交。保留期限、删除权限和数据迁移版本必须由项目确定。
- **合法/拒绝实例**：`('t-7', 2, 'SUCCEEDED', 'result://7')` 在结果已发布时合法；`('t-8', 1, 'SUCCEEDED', NULL)` 违反约束，应拒绝且不得提交部分终态。
- **验证**：`V-EX-JOBS-01` 覆盖约束拒绝、提交失败与崩溃恢复；教学示例 `NOT_RUN`。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**本节目的**：说明主要业务数据及伴随的描述符、元数据在系统各阶段如何产生、变换、保持对应并失效。

**必须写清楚**：阶段 ID、格式 authority、业务数据与必要元数据的生产者、修改者、消费者、变换、关联身份、有效范围、顺序和生命周期。
...
伴随数据的描述符或元数据须说明存在理由、谁产生和更新、谁使用、何时失效，以及如何与
payload 对应。序号要写明有效范围和复用条件；并发、乱序、丢弃或背压适用时，说明为什么
不会将元数据配给错误的 payload，不适用时不凭空增加机制。

本节保留读懂数据路径所需的结构示意和有代表性的值，不复制全部字段规范。已存在机器定义的
字段摘要应标明来源版本、摘要范围与一致性核对方法；布局示意若不表示真实位宽或序列化顺序，
要直接注明。没有机器定义时，将公共结构标为 Proposed，说明已定字段、待定编码、责任方
及形成契约的阶段；不能虚构权威文件或把共同字段留给两个下级分别决定。数据尚在设计时
可以画 Proposed 结构并标记待定项，但不能伪装成已落地格式。
...
因此不会写入目录快照。行号只在该输入副本内有效；报告同时绑定输入摘要，不能把另一文件
的相同行号配给本次记录。只有整批校验成功且选择生成模式，DATA-B 才进入 DATA-C；仅校验模式
结束后释放候选记录及其行号。这个示意把保留、修改、新增和不再传递的信息分别解释了出来。

**完成条件**：阶段 ID 与主流程吻合，复制/拆分/合并及 payload、元数据变化都有说明；描述符与 payload 的关联、失效和回收可判定，生产者与消费者能对应。

</details>

| Data/Metadata ID | 所在阶段 | 格式 authority | 产生者 | 消费者 | 变换 | 关联、有效范围与生命周期 |
|---|---|---|---|---|---|---|

<!-- 表格后放代表对象的前后结构示意，逐段解释业务数据及元数据不变、增加、修改、拆分、失效或删除的内容及原因。 -->
...
**必须写清楚**：按实际形态分类的接口、方向、协议、格式 authority、初始化/握手、流控、超时、错误、版本和兼容性；每个接口在一处写完整参数全集、结果、交互和验证。

**编写规范**：

- 所有跨责任边界的连接都必须有 IF ID；实际名称或调用形式是读者主索引，IF ID 用于追踪。
- 公共字段、具体操作、错误、命令及适用的硬件连接必须存在完整定义；本章是统一查阅入口，
  不以接口族名或计划文件代替契约。引用须定位版本、对象/操作和字段，不仅链接整个目录。
- 已有 OpenAPI、IDL、Schema、寄存器表或接口控制文档保持唯一 authority；集中展示时从固定
  源生成或双向核对。没有机器源时可先在受控设计中完整定义并标 Proposed，后续编码须核对
  迁出与唯一来源。未定公共字段和行为阻塞相关接口设计完成，不阻塞无关工作。
- 区分数据面、控制面、管理面和观测面。
...
**本节目的**：按真实函数名、完整签名或 HTTP/RPC 路由集中定义系统拥有的软件接口，使上下级可直接定位契约与实现责任。

**必须写清楚**：每个接口的用途、提供责任、调用位置、固定机器源、输入/输出 Data/Type ID、前置/权限、校验顺序、逐条件系统 Error ID、成功保证、超时/取消/重复/并发/释放、版本兼容、正常和拒绝实例及验证。配置发布、启动、停止、查询等控制接口也在此逐项成节，不另设控制接口定义表；配置字段仍在 §9.2，生效过程见 §5.4。

**编写规范**：以实际可查找名称为主标题，Interface/Member ID 为稳定追踪键。已有 OpenAPI/IDL/Schema/header 保持唯一机器 authority；本层拥有的接口给同基线阅读视图或精确引用。继承接口只写本层增加的约束和原定义位置，不复制签名。区分“请求已收到”“已受理”“已生效”“已完成”；无响应不等于未执行。多目标控制操作逐目标结果或整体原子性必须明确，不能用一个成功码掩盖部分成功。响应丢失或无法查到结果时规定阻塞、取证或人工出口，不盲目换 ID 重试。具体调用顺序引用 §5 的过程图，不在接口记录外维护第二套操作语义。

**抽象示例**：`PublishConfig(target, revision)` 返回 operation_id 只表示已受理；过期 revision 拒绝且不覆盖当前版本，响应丢失用原 operation_id 查询。

**完成条件**：每个系统拥有的操作都可按固定合同实现；下级无需自行决定公共参数、确认范围或失败后的动作。

</details>

#### `<真实限定函数名、签名或 HTTP/RPC 路由>`

<!-- 编写建议：先给完整、可检索的函数签名或路由定义，再逐参数列数据结构、取值约束和校验失败结果；成功输出及各错误输出分别说明触发条件、系统状态和下一步。系统层只固定跨边界语义与分配，不替下级设计重复私有函数步骤；字段来源引用 §9 的具体结构。 -->

<!-- 先在此放代码式完整声明：函数/端点名称(参数名: 数据结构名, ...) -> 成功数据结构名 | 错误数据结构名；使用项目实际语法、参数名和数据结构名。 -->

- **Interface/Member ID、用途、提供责任与唯一来源**

  <!-- 本层拥有/继承/分配；机器源 selector/version/revision/hash -->
- **输入与前提**

  <!-- 参数/请求 Data/Type ID、范围与跨字段约束、身份权限、校验顺序 -->
- **成功输出与保证**

  <!-- 返回/响应/事件 Data/Type ID、受理/生效/完成及副作用 -->
- **错误与合法下一步**

  <!-- 逐条件 Error ID、载荷、结果已知性、查询/等待/安全重试 -->
- **交互与生命周期**

  <!-- 调用位置/上下文、同步/异步、期限、取消、重复/幂等、并发、释放、兼容 -->
- **实现分配与验证**

  <!-- 提供单元、真实位置或 Planned、正常及拒绝实例、V/Case/Run -->
...
**本节目的**：逐条定义承载数据、事件或配置的流/消息/文件交换，不把格式与传输失败留给两端猜测。

**必须写清楚**：真实 topic、消息/帧/记录或配置输入名，提供和接收边界，Data/Type ID、编码、帧边界、顺序、关联身份、校验、速率/期限、流控与背压、损坏/丢弃/重传、停启和结果。配置文件/启动环境入口还应说明合法来源、优先级、默认/冲突、版本、读取与生效范围、未知键及敏感值；不为补模板新增 fallback 或热更新。

**编写规范**：每条接口集中写全部传输和交互约束；字段全集引用 §9 中对应结构或机器源，公共错误码及错误载荷均引用 §9.1。说明谁能够判定成功、失败或结果未知，何时允许重放，统计口径引用 §11.2。持续流说明缓冲/信用耗尽时的动作，不允许无声覆盖。启动与在线配置生效过程分别引用 §5.3、§5.4，接口记录保留其输入和返回合同。

**抽象示例**：虚构图像流按 frame_id 保序，信用耗尽时暂停发送；损坏帧丢弃并计数。图像字段不在本节重写。

**完成条件**：两端按同一格式、边界和背压/损坏规则互通，正常和异常输入均有可核对结果。

</details>

#### `<真实消息、topic、队列、流或配置入口名称>`

<!-- 编写建议：把消息或数据流的发送/接收边界、完整报文类型、关联 ID、顺序、重复、丢失、确认和背压写在同一个接口小节。配置入口还应说明校验、生效点和在途请求是否继续采用旧配置；不要只画一根“消息总线”箭头。 -->

<!-- 先在此放代码式完整声明：消息/流/配置入口名(输入数据结构名) -> 确认或输出数据结构名 | 错误数据结构名；使用项目实际语法、参数名和数据结构名。 -->
...
- **输入输出与关联身份**
  ...
  <!-- Data/Type ID、编码、边界、校验、确认/结果 -->
- **交互与生命周期**

  <!-- 顺序、速率、流控、背压、重复、期限、停启、版本 -->
- **错误与合法下一步**

  <!-- Error ID、损坏/丢失/超限、结果已知性、丢弃/重传/阻塞 -->
- **配置专有条件（适用时）**

  <!-- 来源、优先级、默认、校验、生效、敏感值 -->
- **实例与验证**

  <!-- 正常及边界帧/文件、V/Case/Run -->
...
**本节目的**：逐连接固定板卡、器件、FPGA 或固件边界；纯软件且不承担硬件连接设计的项目按 tailoring 省略，不虚构电气接口。

**必须写清楚**：真实端点、连接器/管脚 authority、方向、电气电平、标准版本及项目选项、时钟复位、速率/通道、初始化、有效/握手、错误、安全状态、维护/制造测试边界及验证。标准接口不抄整本标准，须明确本产品所选特性；自定义连接补齐信号和不合法组合。

**编写规范**：区分 §6 的板卡组成、此处唯一的互连合同和下级 PCB/RTL 实现。寄存器字段与位域引用 §9.2 或固定 register map；软件驱动函数另在 §10.1 定义并以相关 ID 连接，不复制物理接口内容。未定电气或时序参数是设计缺口，不把“使用 PCIe”当完成。

**抽象示例**：虚构 `READY_N` 是指定电平域的低有效输入，板卡稳定后才可采样；电平、驱动与稳定时间须在正式连接记录中核对。

**完成条件**：每条实际连接具有两端一致的电气和时序边界，联调人员不需自行推测项目选项。

</details>

#### `<真实连接、总线、寄存器组或端口名称>`

<!-- 编写建议：给出硬件/固件边界的接口名、方向、数据或信号类型、握手时序、完成和错误判定及复位状态。引脚、电气、寄存器位域等详细权威引用专项设计或机器定义；系统层必须说清系统依赖的行为保证与验证责任。 -->
...
- **输入输出与物理约束**
  ...
  <!-- Data/Type ID、方向、连接/管脚、位宽、电气、clock/reset -->
- **交互、错误与恢复**

  <!-- 标准选项、时序、握手、流控、异常、安全状态 -->
- **实例与验证**
...
**本节目的**：逐项确定用户与维护者在哪里、以什么身份、对哪个对象执行什么操作，以及如何判定结果和安全退出。

**必须写清楚**：实际 UI 操作、CLI 命令、管理页面、状态查询、统计/日志获取、自检诊断、跟踪、受控测试注入、恢复和升级入口；逐项说明执行主机或页面、目标选择、权限、完整语法/参数/默认/互斥、前置状态、输出/退出码、超时/取消、运行影响、审计、撤销与验证。根据实际产品裁剪，不要求纯软件有指示灯或每类能力有独立命令。

**编写规范**：每条命令/操作在此唯一完整定义；其底层调用的控制 API 引用 §10.1 的成员 ID，不复制 API 参数合同。区分“客户端在哪里运行”和“哪个目标真正执行”。危险动作固定授权、确认、作用范围及退出，禁止默认对全部实例操作。统计口径和日志策略引用 §11.2，自检责任引用 §11.2.1；本节只定义入口的调用和反馈。维护通道不可绕过共同授权和状态检查。

按实际需要核对八类能力：状态、版本与资源查询；配置查看与校验；运行统计；日志与证据获取；自检与诊断；跟踪与捕获；测试注入、替代与环回；恢复与维护操作。可由同一入口承载多类能力，不为表格新增命令。每条接口至少写清命令运行位置与入口、被控对象/选择方式、执行身份与前置条件、关键调用/执行流程、返回与失败判定、运行影响/退出及责任模块/规格/验收。替代依赖/旁路/环回还须说明替代范围、启用权限、命中判据和撤销确认；多任务环境隔离由 §13.4 设计。

**抽象示例**：虚构 `example-maint stats show --target node-a --instance worker-1` 在维护机执行，只读取指定实例；连接或权限失败返回非零退出码，不以全零统计伪装成功，也不改选其他实例。

**完成条件**：非作者能选择正确入口和目标、构造调用、解释成功及失败并安全退出；代表操作具有可执行验证。

</details>

#### `<真实页面操作、命令或诊断入口>`

<!-- 编写建议：先写用户实际执行的页面动作或命令语法，逐输入说明权限、对象、范围和默认值，再写可见反馈、错误、超时、取消及审计。系统设计固定“能做什么”和安全边界，下级文档再细化控件或解析实现；没有此类入口时给出适用性依据。 -->
...
- **执行位置、目标、身份与前提**
  ...
  <!-- 页面/主机/客户端、被控对象、权限及状态 -->
- **输入与操作**

  <!-- 完整语法/控件、参数/默认/互斥、底层 API 成员 ID -->
- **输出、错误与交互**

  <!-- 反馈/退出码、Error ID、期限/取消、作用范围、审计、恢复 -->
- **实例与验证**
...
**本节目的**：检查多个接口组合后是否共同实现同一个系统能力，不重新定义任何单个接口。

**必须写清楚**：实际两端采用的固定版本、Data/Type ID、Error ID、目标身份、单位、顺序、确认与超时语义，以及兼容结论、冲突责任和组合 V/Case。

**编写规范**：沿 §4 的能力或 §5 的过程逐项比较提供和消费的真实技术条款；只有链接可打开或版本号相同，不代表语义兼容。发现冲突必须回到对应接口的唯一合同修正，不能在本表写第三套参数或成功含义。已有接口本身的输入、输出、错误、版本及实例只在 §10.1–§10.4 对应记录中维护；设计核对不能冒充尚未执行的契约或端到端测试。

**抽象示例**：发送方把同名 `OK` 当已生效，接收方只保证进入队列；该组合不兼容，须回到对应接口的唯一合同修正，不能在本表写第三种含义。

**完成条件**：每项公共能力的提供与消费在同一基线上配对，未解决的语义冲突及未运行的组合验证分别可见。

</details>

| 能力/过程 | 提供接口 ID 与基线 | 消费接口 ID 与基线 | 字段/状态/错误对应 | 兼容结论或冲突条款 | 组合 V/Case |
|---|---|---|---|---|---|
...
跨组件使用的日志字段、统计口径和命令语法须按 §10 与本节定义完整共同契约；本地存储实现、
非公共调试细节由下级承接。可观测性要求不仅是“输出日志”，还包括怎样关联和判定结果不完整。
...
经过什么路径、依据什么判断、失败如何报告和影响服务。使用环回或注入时引用 §10.4 的接口，
不在本节重定义接口；破坏性或耗时检查说明维护窗口和退出条件，局部通过不代表整体健康。
明确责任模块与验收依据。检查项目、路径、依赖、结果和恢复协议在以下子节决定，公共命令
由 §10.4 的对应接口记录唯一维护；具体检测算法和现场逐步操作程序由下级设计及操作文档承接。
...
**本节目的**：把“支持自检”分解为能分别实施、判定和解释覆盖范围的检查项目。

**必须写清楚**：检查 ID、目标、触发阶段、检查原理、输入与独立判据、通过/失败/跳过/不支持
结果，以及对业务的影响。无硬件的系统可检查依赖或数据一致性，不套用板卡测试清单。

**编写规范**：从要发现的故障反推检查，不从已有命令名拼出清单。区分上电、启动、在线和
维护检查，说明破坏性、占用资源和允许执行的状态。每项给出确切目标及检查边界，跳过必须
保留原因；不把目标未响应、项目不支持或前置失败合并成通过。检测算法可由下级设计，但
输入、覆盖承诺和判定保证是系统对下游的要求。

**抽象示例**：虚构单元的内部存储读写检查仅在业务关闭时运行；在线查询只读取错误计数，
两者不是等价自检。读写检查未执行时不能以计数为零宣称存储路径已测试。

**完成条件**：清单中的每项都能解释检测什么与漏掉什么、何时允许执行和如何判定；不存在
一个“健康”结果替代多项未执行检查。

</details>

| Check ID | 目标/适用阶段 | 输入与检查原理 | 独立判据/结果枚举 | 业务影响/前置 | 覆盖及不覆盖 | 实现/验证责任 |
|---|---|---|---|---|---|---|
...
**本节目的**：说明检查如何组合，防止前置失效引发误判或共享资源上的并行检查相互干扰。

**必须写清楚**：检查依赖、允许并发的集合、资源互斥、前置失败后的跳过规则，以及输入到
比较点的真实路径。局部、跨组件与外部路径分别标识。

**编写规范**：画简短依赖图或路径表，沿数据源、处理、传输、比较与结果回收逐段说明。环回
和替代依赖引用 §10.4 的唯一接口记录，不在这里重新定义；替代了哪一段，就不能声称覆盖真实
被替代部件。并行依据来自资源和状态独立性，不能仅因两个 Check ID 不同就并行。把未执行
与执行失败区分，保留最早可定位故障，避免下游级联错误掩盖根因。

**抽象示例**：虚构设备先检查控制访问，再执行内部路径回送；控制访问失败时后者标为前置
阻塞，不记为又一条链路故障。内部回送不经过外部线缆，因此不证明外部连接正常。

**完成条件**：每项检查有可执行顺序与路径边界；能判断一处失效会阻止哪些检查，哪些可以
继续，且不会用局部通过代替端到端覆盖。

</details>

| Check ID | 依赖/失败时跳过 | 数据源→经过路径→比较点 | 共享资源/串并行 | 覆盖差异与依据 |
|---|---|---|---|---|
...
**本节目的**：让自检产生可用的故障结论，并安全退出，不把返回报告等同于现场已恢复。

**必须写清楚**：逐目标结果与汇总规则、失败到禁用/降级能力的映射、取消/超时后的动作、
测试资源清理、原配置恢复和重新开放业务的条件，结果保存及责任。

**编写规范**：由 §5.1 指定的运行时统筹者按检查清单汇总，保留部分完成和未知结果。说明
失败仅影响本单元还是共同依赖，按 §11.1 采取动作；“降级运行”必须指出哪些能力禁用和
允许的剩余范围。取消只代表请求，须确认检查不再修改状态、注入已撤销、资源已归还后才
重新开放；控制者退出后的核对规则引用共同协议，不假定 finally 块一定执行。

**抽象示例**：虚构诊断关闭一个输出通道做测试，失败后保持该通道禁用，其余是否继续由
既定依赖规则决定。清理确认丢失时报告恢复未知，不能先开放通道再补查。

**完成条件**：每种汇总状态能追到原始检查结果、实际剩余能力和恢复条件；有清理确认及
中断验证，不存在“诊断结束所以业务健康”的隐含推理。

</details>

| 检查/汇总结果 | 受影响能力与禁用/降级规则 | 清理/恢复动作及确认 | 重新开放条件 | 记录/验证 |
|---|---|---|---|---|
...
**本节目的**：把统计和日志要求变成跨组件可以一致生成、消费和解释的数据契约。

**必须写清楚**：公共指标/事件目录、生产者、单位和范围、计数触发点、累计或区间语义、
复位/溢出、采样窗口、日志必要字段、保留和丢失检测；字段全集按 §9.2 唯一维护。

**编写规范**：从故障定位或容量判定需要出发，解释每个必要信号怎样产生、如何使用，不能
只列“吞吐、延迟、错误日志”。明确请求数与完成数的边界，迟到事件落入哪一个窗口，重启
后的计数怎样区分。定义采样和保留代价以及权限脱敏要求；统计清零与业务状态清除必须是
不同语义，不从命令名推断其作用。

**抽象示例**：虚构指标 `completed_total` 在终态记录确认时累加，不在受理时累加；进程
重启导致累计值重置，读取结果同时带计数代次。跨代次不能直接相减求吞吐。

**完成条件**：两处读到同名信号时单位、窗口、范围和事件边界一致；缺失、重置或溢出可
识别，消费者不用靠猜测重建口径。

</details>

| 指标/事件 ID | 生产者/消费目的 | 触发点/单位/范围 | 累计或窗口/复位/溢出 | 完整字段定义 | 保留/丢失与验证 |
|---|---|---|---|---|---|
...
**本节目的**：规定如何把多处观测拼成可信结论，并明确何时不能拼接。

**必须写清楚**：对象/操作/运行代次关联键、各时钟域与时间精度、观测窗口、采集顺序、
快照一致性和不完整标记。单组件不需要跨节点同步时说明局部时间边界即可。

**编写规范**：沿一次问题定位解释关联字段如何传播及由谁校验；不能仅按相近时间把两条
日志当作同一操作。跨节点时间差必须有同步依据与误差范围，否则保留局部顺序和因果关联。
声明快照是原子截面还是逐目标采样，后者给出采样时间/代次和缺失范围。缺少关键目标时
只能得出有限结论，不能以成功采集的子集冒充全机一致状态。

**抽象示例**：虚构汇总器先后读取两个实例的计数，返回各自采样时刻及代次，并标为非原子
观测。实例重启或未响应时该条不可用于跨窗差分，其他条仍可单独展示。

**完成条件**：一次跨组件诊断能复原关联依据和时间限制；部分采样、代次变化和缺失结果
不会被错误合计或解释成一致快照。

</details>

<!-- 写出关联键及传播路径、时钟/窗口语义、快照一致性、缺失和重启时的消费者行为。 -->
...
涉及多部件时明确版本组合、升级次序、生效检查点和中断后的当前状态判据；可升级不等于
可混用任意版本，恢复时须核对实际生效组合。备份存在不等于恢复已验证，更不等于零数据损失。
回退受 §14.5 信任/撤销规则约束，不允许
为恢复可用性重新启用被禁止版本。公共命令参数在 §10 定义，现场逐步操作由操作文档承接；本节的入口、
...
**本节目的**：确保测试能够确实触发指定行为，而不是测试程序“发出了注入命令”就认为
被测对象已经进入预期状态。

**必须写清楚**：实际需要的控制点、目标及运行身份、启用条件、匹配范围、动作、命中/未
命中确认、结果观测和禁用权限。优先复用已有受控入口，不要求产品新增通用注入平台。

**编写规范**：区分配置已接受、控制点已武装、目标实际命中和动作已撤销；每项确认给出
生产者与可验证状态。明确一次性或持续触发、命中次数、并发冲突、迟到请求，以及取消与
命中并发时的裁决和最终结果。公共操作按 §10 定义，测试方法引用它们，不另立绕过授权的秘密通道。缺少真实命中
证据时记录未触发或未知，不能把自然发生的相似错误当作控制点成功。

**抽象示例**：虚构测试只对指定测试运行的下一条校验请求注入格式错误；接口接受并不代表
已经命中，结果记录要关联该运行和请求。未出现匹配请求就记未命中，不给功能通过结论。

**完成条件**：测试者可以设定、确认命中和撤销指定控制点，且能分辨未支持、未命中与
被测行为错误；运行对象变化不会让迟到注入污染下一次测试。

</details>

| 控制点/能力 | 目标/运行身份/匹配 | 启用前提/权限 | 动作与次数 | 接受/命中/撤销确认 | 冲突/超时/取消 | Oracle |
|---|---|---|---|---|---|---|
...
**本节目的**：限定测试控制的真实覆盖，并保证测试退出不会遗留改变后续行为的状态。

**必须写清楚**：真实硬件/软件、仿真和替代依赖分别支持哪些控制点，未支持时的拒绝行为；
清理责任、触发条件、保存证据、撤销确认和复位后就绪判据。

**编写规范**：按能力而不是只按环境名写支持矩阵。模拟路径通过只证明所声明模型的行为，
不冒充真实设备执行。正常结束、超时、取消、脚本或控制者退出均须设计清理路径，不能只靠
测试脚本善后；无法保证自动清理时保持目标隔离并给出恢复程序。§13.3 说明环境复位，本节
说明控制状态如何被证明确已撤销，两者共享同一资源归属规则。

**抽象示例**：虚构测试环境只在模拟器支持丢包控制，真实设备收到请求明确返回不支持；
模拟器退出后清除本运行配置，再核对控制点为空才进入下一轮，不以新建结果目录代替复位。

**完成条件**：支持与未支持能力能被判别；每条退出路径都有清理或安全阻塞结果，后续任务
不会继承未说明的控制状态，仿真与真实证据始终分开。

</details>

| 控制点 | 真实环境支持 | 仿真/替代支持与差异 | 未支持行为 | 退出/中断清理责任 | 撤销与重新就绪判据 | 验证 |
|---|---|---|---|---|---|---|
...
以 §4.1 的能力子项为覆盖单位，包含适用的管理、诊断、升级、安全与测试支撑，而不只主业务。
各子项关联所有参与方、正常结果、关键失败和恢复判据；可合并等价用例，但注明等价范围。
设计核对、单方契约检查和系统组合验收分别记录，一个代表主链或一张“全部通过”表不能
代替未覆盖的公共操作、诊断项目及跨方结果。能力尚未设计清楚与测试尚未执行须分开。
...
任务按问题性质区分：系统协作方案未定，先做系统预设计；已有局部定义，先核对并纳入组合；
旧机制冲突，先修订适用条款；共同方案及契约已明确，才作为下级实现输入。写明受影响的依赖
边界，已独立确定的部分可以继续。交付按可验收能力子项关联全部提供方和消费者，不以宽泛的
“维护实现”任务或单方完成代表整条链完成。
...
**抽象示例**：虚构采集系统已经确定“旧传输停止后才可复用缓冲区”，共同契约已明确停止确认
所证明的事实、在途资源保留及迟到写隔离规则。下级任务要求驱动与 FPGA 分别落实这些规则，
本地验证各自接口，再组合验证超时后的缓冲复用；内部状态表可由下级选择，不能改变系统保证。
如果共同协议未定，须先做机制预设计，而不是让两端各自补齐。若器件尚不具备可证明的停止能力，则
...
**抽象示例**：`FIG-5-1 数据平面处理流程 — §5.2`；`IF-CTRL-001 配置发布 — §10.1`。
...
§10.3 仅对本系统承担设计或选项选择责任的硬件连接展开；纯软件保留外部环境前提即可。
§5.1.1/§5.1.2 依实际跨组件协作展开，单组件保留本地决定与结果边界；§11.2.1 和 §13.2
的子节仅设计产品实际需要的检查/控制能力，不新增通用平台。条件不成立或合并时按下表记录
依据及信息去向，不能因机制尚未实现而将已承诺能力裁剪掉。
...
- **架构视图**
  ...
  每张图明确 `Current / Target / Transitional`、Baseline ID、适用版本/配置/拓扑。
...
- **能力状态**
  ...
  实现列使用 `Planned / Partial / Implemented`，分别说明未实现、部分范围实现、
...
- [ ] 每项适用能力能定位到运行时统筹、参与方确认和整体/部分失败结果；公共操作、字段、错误、命令和适用连接已完整定义，实际技术条款已核对，不只有 IF ID 或计划链接。
- [ ] 自检项目、依赖/路径和退出恢复、统计关联/窗口/代次、测试控制命中/撤销及真实/模拟支持范围按实际能力展开；一个代表成功案例没有替代全部能力覆盖。
