...
能否误命中新对象必须有判据。复用项目已有定义，公共字段在 §10.5 唯一维护；拓扑变化的
...
**公共错误码**：§9.5 逐码定义系统对外错误的含义、载荷和下级用法。错误码是公共数据结构，不能仅列错误名称或推迟到模块自行决定。
...
**编写规范**：对所有公共操作逐项展开，而不是只提供一个成功请求示例。字段按 §10.5 定义，
...
决定，跨组件寄存器位域按 §10.5/§10.6 固定共同契约，不能留给两端各自猜测。
...
**本节目的**：固定跨组件对象的可交换表示，避免名称一致但类型、单位或生命周期不同。

**必须写清楚**：对象/结构 ID 和版本、全部公共字段的类型、长度、范围、单位、必选性、默认
值、枚举/保留值、读写责任及关联关系；二进制结构还须给字节序、对齐、长度和校验规则。

**编写规范**：身份、描述符、配置、结果、统计和日志等按实际共享范围定义，不强制统一成
一种大对象。逐字段核查缺失、空值、未知值、越界与版本差异的行为，时间标注时钟域，数量标注
作用域。已有机器定义为唯一来源，本节给固定版本和精确对象位置；用户要求集中字段表时生成
或核对展示副本。局部私有缓存布局不属于公共契约，不把它们无差别搬进系统设计。

**抽象示例**：虚构测量结果用 `duration_us: uint32` 表示一次操作耗时，范围为 0～4294967295；
测量缺失另用有效性字段表达，不约定“0 代表未知”。这是字段写法示范，实际结构须补全全部字段。

**完成条件**：两个实现者能独立生成并解析相同字节或数据对象；异常值、单位、所有权和版本
有一致解释。所有公共结构完整可定位，未定义字段仍记录为设计缺口。

</details>

| 结构/版本/唯一来源 | 字段 | 类型/编码/长度 | 范围/单位/枚举 | 必选/默认/异常值 | 读写责任/生命周期 | 消费者与验证向量 |
|---|---|---|---|---|---|---|
...
完整字段引用 §10.5 的唯一配置契约，启动与在线生效过程分别引用 §5.3/§5.4。说明某输入只
...
复位/溢出、采样窗口、日志必要字段、保留和丢失检测；字段全集按 §10.5 唯一维护。
...
§10.6 仅对本系统承担设计或选项选择责任的硬件连接展开；纯软件保留外部环境前提即可。
