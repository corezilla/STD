...
**公共错误码**：§9.8 逐码定义系统对外错误的含义、载荷和下级用法。错误码是公共数据结构，不能仅列错误名称或推迟到模块自行决定。
...
<!-- 编写建议：只收录本层拥有或必须呈现的公共基础类型与枚举；已由上级或机器源定义的值只引用固定来源。逐值写含义、范围、未知值处理和适用边界。虚构例子：TaskState 的 RUNNING 只表示任务已开始执行，不等于完成。 -->
...
**编写规范**：每条接口集中写全部传输和交互约束；字段全集引用 §9 中对应结构或机器源，公共错误码引用 §9.8。说明谁能够判定成功、失败或结果未知，何时允许重放，统计口径引用 §11.2。持续流说明缓冲/信用耗尽时的动作，不允许无声覆盖。启动与在线配置生效过程分别引用 §5.3、§5.4，接口记录保留其输入和返回合同。
