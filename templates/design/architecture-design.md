...
API 须明确其所属模块，不能把抽象接口、硬件接口和操作系统当作同级软件组成。
...
**本节目的**：当系统包含 WebUI 时，说明软件架构中的哪些对象承接 §10.4.2 已定义的页面、操作和状态反馈；页面设计的唯一位置是 §10.4.2。

**必须写清楚**：WebUI 软件对象、页面路由和 Page ID 的实现归属、后端契约映射、前端状态管理与服务端授权的边界，以及交给下级的软件实现约束。

**编写规范**：沿 §10.4.2 的页面导航图、线框图、重要流程和逐动作记录，按 Page ID 映射到
实际 WebUI 软件对象及后端 Interface/Member ID。本节只解释这些决定如何在软件架构中落实：
路由与状态归属、查询/命令适配、跨页状态清除、结果未知的查询入口、刷新与取消怎样保持原
任务身份，以及服务端授权在何处独立执行；不在此维护第二份页面布局、按钮行为或错误码定义。
系统层定义共同边界，下级 UI 模块可细化组件和视觉实现，不得改变 §10.4.2 的业务反馈语义。
无 WebUI 时按 §B.2 记录 N/A、理由和裁剪决定，不为满足模板新增页面。

**抽象示例**：虚构 WebUI 对象承接 §10.4.2 的 TaskList 与 TaskDetail 两个 Page ID，使用
同一任务查询成员获取权威状态。刷新或路由返回时按原任务身份重新读取，不把页面缓存当作
完成事实；提交结果未知时调用原请求查询成员，取消入口仅显示请求状态，直到服务端确认终态。
仅有查看权限时，前端隐藏修改操作，服务端仍独立校验授权。页面布局和完整用户流程只在
§10.4.2 定义，此处只固定软件承接和边界。

**完成条件**：每个主要 Page ID 均能定位承接的软件对象、后端接口和状态/授权边界；实现者
不需重新决定页面业务语义，§10.4.2 的图与操作能落实到软件设计，无 WebUI 的裁剪有依据。

</details>

| Page ID / §10.4.2 图引用 | 承接软件对象 | 查询/命令成员 | 状态、路由与授权边界 | 下级验证 |
|---|---|---|---|---|

<!-- 编写建议：按 §10.4.2 的实际 Page ID 逐项映射软件承接对象与后端契约；不要在此重写页面布局、线框图或交互流程。 -->
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.1.1 `InspectionErrorCode`（公共错误码枚举，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.1.2 `InspectionError`（错误载荷结构，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.1.3 `InspectionState`（公共状态枚举，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.2.1 `InspectionRequest`（业务对象，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.2.2 `InspectionSubmission`（受理结果，虚构）**
...
- **约束与生命周期**：
  ...
  受理责任单元在任务身份持久确定后构造结果；响应丢失时仍可凭原 `request_id` 查询。任务何时完成由 §9.1 的 `InspectionState` 表达，不能从 `InspectionSubmission` 推断成功检测。
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.2.3 `InspectionRuntimeState`（运行状态，虚构）**
...
- **Data/Type ID、用途与来源**：
  ...
  `DATA-EX-RUNTIME`；表示一项任务当前权威状态。示例机器源为 `Proposed: InspectionRuntimeState`；`InspectionState` 指向 §9.1 的同名教学类型，真实项目须改用自己的唯一来源。
...
- **`state`**：
  ...
  必填，取 §9.1 `InspectionState` 的已定义值。
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.3.1 `config/inspection-policy.yaml`（配置文件，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.5.1 `CaptureStatusRegister`（设备寄存器，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**9.5.2 `RouteTableEntry`（FPGA 数据表项，虚构）**
...
接口声明中的类型名必须能直接定位到前述“数据结构设计”章或固定的外部机器来源；不要只给 ID 让读者猜输入输出。完整的 `submit_inspection` 教学记录放在 §10.1，所用请求、成功输出和错误载荷分别由 §9.2、§9.1 唯一定义，不在本章维护第二份字段表。
...
**本节目的**：按真实函数名、完整签名或 HTTP/RPC 路由集中定义系统拥有的 API，使上下级可直接定位契约与实现责任。
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**10.1.1 `submit_inspection`（虚构异步受理接口）**
...
- **Interface/Member ID、用途、提供责任与唯一来源**:
`IF-EX-SUBMIT-INSPECTION`；受理一项图像检测请求，由 `InspectionAdmission` 提供。接口和三个类型均为教学用 `Proposed` 契约；输入与成功输出字段见 §9.2 的 `InspectionRequest`（`DATA-EX-REQUEST`）、`InspectionSubmission`（`DATA-EX-SUBMISSION`），错误码和载荷见 §9.1 的 `InspectionErrorCode`、`InspectionError`（`DATA-EX-ERROR-CODE`、`DATA-EX-ERROR`）。正式项目须换成固定的机器源、版本和成员 selector。

- **输入与前提**
	* `request`：必填,类型 `InspectionRequest`。其中 `request_id` 必须是本次业务请求的稳定身份；`image_uri` 必须指向调用身份有权读取的有效图像；`policy_version` 必须指向已发布规则。先检查身份权限，再检查请求字段、图像引用和规则版本；之后以 `request_id` 原子核对已受理记录：内容相同走重放，内容不同拒绝；只有新请求才检查队列容量并创建任务。字段的类型和跨字段约束只在 §9.2 定义，此处写该接口如何使用它们。
...
- **错误与合法下一步**：
输入字段或引用无效时返回 `InspectionError{code:INVALID_REQUEST, request_id:<原值或 null>, task_id:null}`，调用方修正请求；同一 ID 携带不同内容时返回 `REQUEST_CONFLICT`，调用方核对原请求，不换 ID 绕过；未获授权时返回 `PERMISSION_DENIED`，须先取得授权；新请求遇队列满返回 `QUEUE_FULL`，本次未受理，可稍后以同一请求身份再试。四种错误均不创建本次任务，码值含义和载荷约束由 §9.1 唯一维护。
...
**本节目的**：逐条定义组件或系统为协作而交换的命令、状态、事件、流、文件等通信接口，不把格式与传输失败留给两端猜测；内部协作即使使用 HTTP/RPC，也在本节定义，面向使用方的 API 留在 §10.1。

内部协作使用 HTTP/RPC 时，以实际 method + route 或 RPC 方法为标题，在同一记录说明请求/响应、鉴权、协议状态与业务错误映射、超时和版本；不得因传输形式把同一接口再放入 §10.1。
...
**编写规范**：按 §10.4.1 CLI、§10.4.2 WebUI 展开实际入口；没有其中一种入口时说明裁剪依据，不为满足模板新增界面。每条命令/操作在此唯一完整定义；其底层调用的控制 API 引用 §10.1 的成员 ID，不复制 API 参数合同。区分“客户端在哪里运行”和“哪个目标真正执行”。危险动作固定授权、确认、作用范围及退出，禁止默认对全部实例操作。统计口径和日志策略引用 §11.2，自检责任引用 §11.2.1；本节只定义入口的调用和反馈。维护通道不可绕过共同授权和状态检查。
...
**本节目的**：定义操作者或维护者能够实际执行的命令，以及命令作用对象、结果和安全边界；不能只列命令名称。

**必须写清楚**：CLI 运行主机、工作目录或连接上下文、执行身份、目标选择方式；按真实命令逐项给完整语法、参数/默认值/互斥关系、前置状态、权限、调用的接口成员、正常输出、错误信息与退出码、超时/取消、运行影响和审计。没有 CLI 时记录 N/A、理由和 tailoring 决定。

**编写规范**：先按用户任务分组，再以真实命令名作为记录标题；一条命令的语法、行为和所有结果集中写在同一记录内。读取类命令不能在连接失败时输出看似正常的空值；修改类命令必须指定目标、授权、确认与生效/回滚边界，不得默认广播。底层 API 的字段和错误值引用 §10.1、§9.1 的唯一来源，命令记录只说明 CLI 对这些结果的呈现与退出码映射。CLI 与 WebUI 执行同一业务操作时引用相同业务契约，不分别制定成功条件。

**抽象示例**：虚构 `inspectctl status --task t-7` 在维护机执行，读取指定任务并输出状态及更新时间；任务不存在返回已定义错误和非零退出码，连接失败不伪装为 `UNKNOWN` 业务状态。这个示例不表示项目必须提供该命令。

**完成条件**：非作者能在指定环境构造命令、选中正确对象、解释输出与退出码，并知道失败、结果未知或中途停止后的合法下一步；代表正常与拒绝路径可验证。
...
- **Interface/Member ID、用途、提供责任与唯一来源**：
  ...
  `CLI-TASK-STATUS`；读取单个任务的权威状态。CLI 由 `TaskCli` 提供，调用 §10.1 的 `QueryTask`；任务状态类型和错误类型以 §9.1 的 `TaskStatus`、`SystemError` 为唯一来源，本例不重复定义字段或错误码。

- **执行位置、目标、身份与前提**：

  操作者在已配置服务端地址的维护终端执行；`--task` 必填且只选一个任务，不允许空值代表“全部”。使用当前登录身份，需具有该任务的读取权限；目标服务地址取已选配置，不因连接失败自动切换环境。
...
- **输出、错误与退出**：
  ...
  成功时输出任务 ID、状态和权威更新时间，退出码 `0`。任务不存在显示 `TASK_NOT_FOUND`，退出码 `4`；无权读取显示 `PERMISSION_DENIED`，退出码 `5`；连接失败或超时分别显示 `UNAVAILABLE` / `DEADLINE_EXCEEDED`，退出码 `6` / `7`，不得输出看似正常的空状态。中断查询只停止本次等待，不改变任务状态；操作者可用同一 ID 再次查询。

- **运行影响、审计与验证**：

  无业务写入；按 §11.2 记录查询审计，不记录敏感任务内容。验证项 `V-CLI-STATUS-01` 分别检查成功、任务不存在、无权限、连接失败和超时，确认失败时均没有正常状态输出。
...
项目成文时应替换命令、成员 ID、数据结构和错误映射；不能把这些教学占位值当作本项目既有契约。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
**本节目的**：在系统层决定用户在哪里进入 WebUI、看到什么、怎样完成主要任务，以及界面如何准确呈现系统事实；不能只放页面名称、WebUI 架构方框或无行为说明的效果图。

**必须写清楚**：

1. **信息架构与导航**：提供页面导航图，区分全局入口、主要页面、页签、详情及外部界面；登记稳定 Page ID、用户任务、角色、入口和返回路径。说明跨页保留的对象/筛选/草稿及切换项目、账号或租户时应清除的上下文。页面不是正式软件模块。
2. **共享框架与页面布局**：提供共享框架布局图，以及每个主要页面的布局或线框图；图后逐区域说明显示的信息、主要操作及依据。按实际需要描述导航、当前对象、业务主区、详情和反馈区，不强制固定栏数。列表说明筛选、排序和大数据量处理；表单说明校验、确认、失败后输入保留。
3. **重要用户任务**：每个重要 UI 流程有流程图或时序图，配正文和异常分支，覆盖触发、校验、提交、受理、生效及返回/恢复。已在 §5 完整表达的流程可引用同一图，但须补用户可见步骤；点击提交不等于业务完成，按钮隐藏或弹窗确认不等于后端授权。
4. **数据、状态与操作映射**：逐主要区域/动作关联权威数据、查询/命令成员及版本、权限和验证项；完整字段仍由 §9 的唯一来源维护。区分 Loading、合法 Empty、Error、Partial、Stale、无权限、提交中、冲突与结果未知，说明局部失败及更新时间。只读刷新不触发业务操作，超时不擅自新建有副作用请求。
5. **跨页面一致性与可用性**：明确重复点击、并发修改、晚到响应、返回/刷新和登录失效时的行为；跨作用域切换不得回显旧对象数据。嵌入外部页面时说明身份、选择范围和故障边界。基本键盘操作、焦点和错误定位可用，状态不只靠颜色；说明支持平台、屏幕尺度及必要本地化边界。

**编写规范**：系统阶段确定页面结构、主要内容、用户流程、反馈和业务边界，不把这些决定全部推给软件下级；颜色、字体、像素和控件实现留给详细 UI 设计。每张图标明 Page ID、角色/场景与 Current/Target 状态，保留可编辑图源。页面动作的接口声明、成功/失败语义在本节唯一维护，底层 API 与数据结构分别引用 §10.1 和 §9，不建立影子契约；§7.4 只说明软件对象如何承接这些决定。没有自有 WebUI 时记录 N/A、理由和 tailoring 决定，不虚构页面。

**抽象示例**：虚构任务产品的 TaskList 展示任务及更新时间，进入 TaskDetail 后返回时保留筛选；TaskCreate 校验后提交，响应丢失时显示结果未知，并按原请求身份查询而非直接再次创建。空列表与查询失败分别呈现；切换工作空间后，旧查询响应不能覆盖新页面。导航图、共享框架图、页面布局图和提交时序图共同解释正常、拒绝与响应丢失路径，不把这套页面强加给所有产品。

**完成条件**：新读者可依据图和正文完成主要用户任务；所有主要页面有布局图，重要 UI 流程有图与异常出口，按钮、状态、权限、数据来源和后端契约一致。原型截图和模板检查通过不等于浏览器行为或可用性验证通过。
...
以下三张图直接收录在本模板的图例资源中，示范如何先确定页面关系和共享布局，再逐个展示主要页面的信息分区；图中的 LLMTier 产品名称、五页两抽屉、功能与状态仅供教学，不是新项目的默认设计或运行证据。项目成文时应替换为本项目的 Page ID、页面、用户任务及实际状态。

![LLMTier WebUI 页面导航与抽屉关系](../diagrams/llmtier-webui-nav.png)

图 EX-UI-NAV · 导航图让读者看到主要页面从哪里进入，以及抽屉归属哪个页面；[可编辑 SVG 源](../diagrams/llmtier-webui-nav.svg)。

![LLMTier WebUI 共享框架布局](../diagrams/llmtier-webui-frame.png)

图 EX-UI-FRAME · 框架图说明导航、页头、当前页面和反馈区的层次；它不是像素级视觉规范；[可编辑 SVG 源](../diagrams/llmtier-webui-frame.svg)。

![LLMTier WebUI 主要页面与抽屉布局](../diagrams/llmtier-webui-page-layouts.png)

图 EX-UI-PAGES · 主要视图按 Page ID 展开页面及抽屉的内容分区；正式系统设计仍须在图后逐区域说明信息、操作、状态与错误反馈，不能用图代替正文；[可编辑 SVG 源](../diagrams/llmtier-webui-page-layouts.svg)。
<!-- STD_TEMPLATE_EXAMPLE_END -->
...
<!-- 编写建议：先给页面导航图和共享框架图，再按 Page ID 展开主要页面布局及区域行为；每个重要用户任务配流程图、正常/异常正文和接口映射。 -->
