...
从 §9.8/§12.1 等推导，分清单实例和整机、独占和共享、常驻与峰值、并存副本、公共开销与
...
**公共错误码**：§9.10 逐码定义系统对外错误的含义、载荷和下级用法。错误码是公共数据结构，不能仅列错误名称或推迟到模块自行决定。
...
**本节目的**：让每个关键数据对象都有稳定身份、明确归属和可定位的定义。

**必须写清楚**：Data/Type ID、用途、定义或引用关系、唯一来源及固定版本、下级承接。

**编写规范**：本节只建索引，不复制字段。对本层不拥有的结构标明真正的定义处；缺少结构时回到上级决定，不让下级各自发明。

**抽象示例**：系统定义结果对象 `DATA-RESULT`，软件层承接其逻辑字段，设备状态引用已有机器类型。

**完成条件**：从任一清单行都能找到唯一字段定义和适用基线。

</details>

<!-- 编写建议：列出本层拥有、继承和分配给下级的全部关键 Data/Type ID，说明用途、唯一来源与版本；清单是索引，不替代 §9.2 的定义。不存在新增结构时仍列承接对象及适用性依据。 -->

<!-- 本章与 §10 使用同一接口目录和类型成员 ID；完整阅读视图绑定机器源版本/revision/hash。共享类型引用原定义，转换/投影另说明损失或补足；范围、单位、默认/null 等差异必须回到唯一源解决。 -->

| Data/Type ID | 用途与适用范围 | 定义/引用 | 唯一来源、selector 与版本 | 下级承接 / 设计验证项 |
|---|---|---|---|---|
...
**本节目的**：固定总体系统拥有的公共对象语义，使下级实现不再猜字段。

**必须写清楚**：逐字段类型、单位、范围、必填/空值/默认、条件有效性、键和跨字段约束。

**编写规范**：机器源已有定义时固定版本与 selector，阅读视图必须生成或逐项核对。继承对象只引用原 ID；Proposed 对象标出机器化责任和截止 Gate。

**抽象示例**：结果 ID 保留前导零，空结果与结果尚未知不能共用同一字段值。

**完成条件**：两个下级单元能依同一来源构造和拒绝相同对象。

</details>

<!-- 编写建议：只完整定义本层拥有的结构：逐字段给类型或引用、必填/可空/默认、范围/枚举/单位、长度或计数、条件有效性与含义，并写明键及跨字段约束。已有 Schema/IDL/位域时固定机器源及 selector，字段阅读视图从该源生成或逐项核对；继承结构只引用 ID 和来源，不复制权威定义。系统公共结构的编码与跨单元互通另在 §9.9 展开。 -->

**9.2.N `<Data/Type ID>` · <结构名称>**
...
**本节目的**：定义对象从产生到失效的责任和事实边界。

**必须写清楚**：创建、唯一写入、读取、可见、失效、释放及失败后保留的事实。

**编写规范**：状态与对象使用 §9.1 的相同 ID；区分临时副本和权威事实。没有持久化仍需说明在途对象的寿命，不把超时等同于执行停止。

**抽象示例**：候选快照仅写者可见，发布后读者才切换，新旧副本的释放以在途读者退出为条件。

**完成条件**：正常及失败路径均有明确所有者、可见点和释放出口。

</details>

<!-- 编写建议：对 §9.1 每个关键对象说明谁创建、修改和读取，何时可见、失效及释放；标明状态权威、并发和失败后保留的事实。状态转换引用同一 Data/Type ID；不适用的持久化或共享状态写出理由，不以“无数据库”代替生命周期分析。缓存、存储及恢复策略在 §9.7 深化。 -->
...
**本节目的**：用具体对象检验字段、状态和下级承接能否实际执行。

**必须写清楚**：合法实例、边界/拒绝实例、预期判定、固定来源及设计 V/Case。

**编写规范**：实例要触发至少一个跨字段或生命周期条件；它是检查向量，不替代字段权威。未实现、未执行和不适用分别记录，不把示意数据写成实测。

**抽象示例**：相同 ID 配不同输入被拒绝，原操作保持已受理状态，并引用相应公共错误码。

**完成条件**：下级和评审者用同一输入得到相同判定，验证状态真实可查。

</details>

<!-- 编写建议：对本层拥有的关键结构至少给一条合法实例及一条边界/拒绝实例，核对字段和跨字段约束、生命周期、来源版本与设计 V/Case。引用结构的实例应定位权威来源；示例不是另一份字段规范。尚未实现或未执行的验证明确标状态，不写成已通过。 -->
...
公共字段仍待定时，相关接口设计尚未完成；由系统/机制补齐共同定义并在 §10 关联后，才能
按同一契约分配给两端实现，不能仅把缺口转交给两个下级各自决定。
...
**本节目的**：解释 §9.2 已定义的公共结构如何编码、传送和转换，避免两端使用同名对象却不能互通。

**必须写清楚**：对象/结构 ID 与固定来源、实际编码和长度边界、版本及未知值行为、投影/转换的保持或损失；适用二进制结构还须给字节序、对齐和校验规则。

**编写规范**：字段全集、单位与不变量仅在 §9.2 和机器源维护，本节用同一 ID 引用，不建立第二份字段表。说明 JSON/二进制等实际载体如何表示空值、未知值、越界和版本差异；有投影时标明原类型、变换责任及无法还原的内容。身份、描述符、配置、结果、统计和日志按实际共享范围分别处理，局部私有缓存布局不属于公共契约。

**抽象示例**：`DATA-DURATION` 的字段约束在 §9.2；本节说明 JSON 数值和二进制 `uint32` 的转换检查，超出二进制范围时拒绝而非截断。这个转换示意不重复完整字段定义。

**完成条件**：两个实现者能依据同一字段来源生成和解析相同数据；编码、版本和转换失败可判定，未定义字段仍记录为设计缺口。

</details>

| Data/Type ID / 固定来源 | 编码与长度边界 | 版本 / 未知值行为 | 投影、转换及损失 | 设计 V / Case |
|---|---|---|---|---|
...
**本节目的**：把跨接口共用的错误码作为公共数据结构一次定义，并使下级能够一致地产生、透传、转换和消费。

**必须写清楚**：每个适用的对外码的代码值、唯一含义、触发事实、判定者、错误载荷、操作结果及副作用、调用方动作、下级承接和验证；列出接口成员与下级设计的对应关系。

**编写规范**：机器 Error enum、Schema 或 catalog 拥有实际代码值、类型与编码；正文固定来源版本/revision/hash/selector并解释公共语义。接口逐失败条件引用 Error ID，下级不得私自新增或改变公共码。私有异常在系统边界前明确映射；没有公共错误码时给出边界和正式 tailoring 依据，不填空表。

**抽象示例**：`ERR-REQUEST-CONFLICT` 表示同一请求标识对应不同输入；入口模块比较已受理记录后拒绝新输入，原操作状态不变。调用方不得把该码当成超时重试；实际代码值与载荷字段由固定机器源定义。

**完成条件**：每个对外错误均能由具体事实触发，找到唯一机器定义、接口使用位置、下级责任和独立验证项；同码异义及无来源码为阻断项。
...
- **Interface/Member ID、用途、提供责任与唯一来源**：<!-- 本层拥有/继承/分配；机器源 selector/version/revision/hash -->
- **输入与前提**：<!-- 参数/请求 Data/Type ID、范围与跨字段约束、身份权限、校验顺序 -->
- **成功输出与保证**：<!-- 返回/响应/事件 Data/Type ID、受理/生效/完成及副作用 -->
- **错误与合法下一步**：<!-- 逐条件 Error ID、载荷、结果已知性、查询/等待/安全重试 -->
- **交互与生命周期**：<!-- 调用位置/上下文、同步/异步、期限、取消、重复/幂等、并发、释放、兼容 -->
- **实现分配与验证**：<!-- 提供单元、真实位置或 Planned、正常及拒绝实例、V/Case/Run -->
...
**编写规范**：每条接口集中写全部传输和交互约束；字段全集及公共错误码只引用 §9.2/§9.10 或机器源。说明谁能够判定成功、失败或结果未知，何时允许重放，统计口径引用 §11.2。持续流说明缓冲/信用耗尽时的动作，不允许无声覆盖。启动与在线配置生效过程分别引用 §5.3、§5.4，接口记录保留其输入和返回合同。
...
- **Interface/Member ID、用途、责任与唯一来源**：<!-- TODO -->
- **输入输出与关联身份**：<!-- Data/Type ID、编码、边界、校验、确认/结果 -->
- **交互与生命周期**：<!-- 顺序、速率、流控、背压、重复、期限、停启、版本 -->
- **错误与合法下一步**：<!-- Error ID、损坏/丢失/超限、结果已知性、丢弃/重传/阻塞 -->
- **配置专有条件（适用时）**：<!-- 来源、优先级、默认、校验、生效、敏感值 -->
- **实例与验证**：<!-- 正常及边界帧/文件、V/Case/Run -->
...
- **Interface/Member ID、用途、双方端点与唯一来源**：<!-- TODO -->
- **输入输出与物理约束**：<!-- Data/Type ID、方向、连接/管脚、位宽、电气、clock/reset -->
- **交互、错误与恢复**：<!-- 标准选项、时序、握手、流控、异常、安全状态 -->
- **实例与验证**：<!-- 正常与边界时序、V/Case/Run -->
...
- **Interface/Member ID、用途、提供责任与唯一来源**：<!-- TODO -->
- **执行位置、目标、身份与前提**：<!-- 页面/主机/客户端、被控对象、权限及状态 -->
- **输入与操作**：<!-- 完整语法/控件、参数/默认/互斥、底层 API 成员 ID -->
- **输出、错误与交互**：<!-- 反馈/退出码、Error ID、期限/取消、作用范围、审计、恢复 -->
