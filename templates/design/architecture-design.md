...
在上述产品场景内选常见、容量边界和异常任务，使用稳定场景 ID 关联功能、过程、§5.2 约束
...
组成职责与 Owner 在 §5.2 说明，端到端过程在 §6 展开；本文示例不是完整工程设计。
...
通过图后正文和 §6 的过程图说明，不要求一张图同时承载组成、数据流和时序。
...
§5.2 有职责与 Owner，主要协作在正文或 §6 可追踪。数据流图中的跨界连接须关联协议或 IF ID，
...
从 §10.4/§13.1 等推导，分清单实例和整机、独占和共享、常驻与峰值、并存副本、公共开销与
...
**FPGA**运行 §9.1 的自研采集与传输逻辑；在这张板卡图中它是一个器件，不展开内部 RTL。
...
及启动顺序另在 §6.2、§7.3 展开。**电源区**包含输入保护、电源转换和监测，本图不画完整电源树；
...
本图展开 §5.1 中的检测应用与配套驱动，不再重复相机、流水线和显示器。
...
普通操作系统不在本软件组成图中作为模块展开，只在 §8.6 记录运行环境；没有系统定制时
...
正式设计还须在 §8.2–8.3 明确进程/线程划分、图像生命周期、并发及失败传播，并在 §8.6 固定
...
图中的帧队列是逻辑职责，不等同于 §7.1 的 DDR 芯片。若采用图示的外部帧缓存，正式设计
...
FPGA 器件内部的物理放置。主要模块图后逐一解释职责，§9.2–9.4 继续展开状态、存储、背压、
...
将模型导出的单元预算、共享资源和公共保留通过 §5.2 分配，下级承接后按同一基线校核组合；
...
把 §5.2 分配的 Constraint ID 纳入目标映射，说明哪项由下级证明、哪项还要系统组合验证，
...
敏感数据或可修改运行状态，其权限、出厂禁用/封闭与受控返修恢复须引用 §11.4/§15，不能成为
绕过生产权限的后门。将必须预留的能力按 Constraint ID 纳入 §5.2；制造测试不能替代未覆盖
的设计验证、可靠性试验或真实环境证据。
...
选择方案后，回写受影响章节、§5.2 分配及下级引用；推荐未获确认时仍保持拟议，不提前关闭。

**抽象示例**：关键器件供货尚未锁定，可能影响量产；Owner 在设计冻结前确认替代方案和验证计划。

**完成条件**：每项风险或假设失效有触发、影响、具体下一步、Owner 和关闭证据/Gate，关键
设计问题有分析路径和受影响工作边界。机制未定不判设计完成；未实现/未实测单独记录，
关闭后的决定已经写回正文与承接关系，不只在问题表中标记 Done。

</details>

| ID | 类型 | 描述与触发条件 | 影响/阻塞与可继续范围 | 预设计/分析位置 | 具体下一步/所需输入/选择判据 | Owner | 关闭证据/Gate/回写位置 |
|---|---|---|---|---|---|---|---|
...
|---|---|---|
| §7 硬件实现 | 本系统拥有板卡、器件或电气连接设计责任 | 外部硬件的依赖、资源及故障边界写入 §2.3/§5.3/§8.6 |
...
选定方案后把原理、关键约束及验证方法写回本节，重大决定关联 §18.1，分析作为依据保留引用。
