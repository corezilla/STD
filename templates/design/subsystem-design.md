...
## 6. 接口设计与 interfaces 映射

<details>
<summary>本节编写建议、规范与示例</summary>

**接口与错误承接**：先列本层提供、消费和分配给直属模块的全部 Interface/Member ID 及固定机器来源；逐成员描述输入、输出、错误 ID、方向、前提、权限、版本与验证。系统公共码按上级定义产生、透传或映射，私有接口不冒充公共接口。
...
**抽象示例**：上级软件系统 SW-P 定义 MOD-D 与 SUB-C 之间的配置交接，双方引用同一来源；本软件子系统 SUB-C 定义自身直属模块间的工作交接，模块私有函数由模块设计决定。SYS-EX 的公共管理语义继续在总体系统契约维护，不在软件子系统重新定义。
