...
**本节目的**：标识子系统内部及边界对象的定义责任。

**必须写清楚**：Data/Type ID、用途、来源版本、拥有或继承关系、直属模块承接。

**编写规范**：清单与上级 ID 对齐，内部共享对象只能由一处定义。不要把状态名当成完整数据对象，也不要在此复制机器字段。

**抽象示例**：任务对象由子系统定义，模块 A 写入结果引用，模块 B 只读取已发布版本。

**完成条件**：每项重要对象的字段定义和读写责任都可定位。

</details>

<!-- 编写建议：列出本子系统拥有、继承及分配给直属模块的 Data/Type ID、用途、唯一来源/版本、定义或引用关系。下表只索引对象；不得以状态名称代替字段定义，也不得复制上级公共结构。 -->

| Data/Type ID | 用途与适用范围 | 定义/引用 | 唯一来源与版本 | 直属模块承接 / 验证项 |
|---|---|---|---|---|
...
**本节目的**：确定直属模块共同使用的数据契约。

**必须写清楚**：字段类型、范围/单位、必填/空值、条件有效性、键及跨字段约束。

**编写规范**：只定义本层拥有的对象；引用上级结构时固定 ID/版本/selector。私有容器选择留模块设计，但共同字段不可写“由实现决定”。

**抽象示例**：任务结果引用只有任务状态为 completed 时有效，其他状态不得填入旧结果。

**完成条件**：两个直属模块可以按同一规则解释合法及拒绝值。

</details>

<!-- 编写建议：本层拥有的关键结构逐字段写类型或引用、必填/可空/默认、范围/单位/枚举、条件有效性与含义，并定义键、关系和跨字段不变量。已有机器源固定版本与 selector 并核对阅读视图；继承对象只引用原 ID。私有容器实现留给模块，不把共同字段下放给双方猜测。 -->

#### 5.2.N `<Data/Type ID>` · <结构名称>

- **定义与适用范围**：<!-- TODO -->
- **唯一来源 / 版本 / selector**：<!-- TODO；或 Proposed 与关闭 Gate -->
- **字段及约束**：<!-- 字段、类型/引用、必填/空值/默认、范围/单位、条件有效性 -->
- **键与跨字段不变量**：<!-- TODO -->
...
**本节目的**：使子系统对象从创建到释放的状态事实连续可查。

**必须写清楚**：唯一写者、可见点、Guard 事实来源、迟到处理和释放条件。

**编写规范**：每条状态转移必须说明观察事实和执行者，不能依赖测试直接改内部状态。中断与超时不自动证明旧执行已停止。

**抽象示例**：任务取消后等待执行适配确认隔离，确认前结果保持未知且资源不复用。

**完成条件**：失败、取消及迟到路径都不会产生无主状态或悬空引用。

</details>

<!-- 编写建议：逐对象说明唯一写入、读取、可见、状态转换、失效及释放；Guard 必须有可获得的事实来源，失败和迟到结果不得写入已释放对象。与 §5.1 清单及 §7 流程使用同一 ID；无持久化仍需分析在途和易失状态。 -->

### 5.4 示例与验证

<details><summary>本节编写建议、规范与示例</summary>

**本节目的**：以实例证明对象约束能够向模块与测试传递。

**必须写清楚**：合法和拒绝实例、预期状态、机器源基线及本地/组合 V/Case。

**编写规范**：实例应覆盖至少一个字段关系或生命周期边界；未实现、未运行不可写 PASS。上级类型的完整字段仍从原来源取得。

**抽象示例**：同一任务 ID 的完成通知重复到达时只保留一次结果，跨 generation 通知被拒绝。

**完成条件**：直属模块和组合验证能得到相同判定与状态出口。

</details>

<!-- 编写建议：对本层拥有的关键结构给合法与边界/拒绝实例，核对字段、状态、所有权及寿命，绑定设计 V/Case 和机器源基线；未实现与未执行分开记录。引用上级公共类型时链接原实例，不复制定义。 -->
