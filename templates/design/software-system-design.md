...
**编写规范**：本模板为《软件系统设计说明书》，Template ID 为 design.software-system。纯软件项目可直接以本文作顶层设计，不强制另建重复的总体系统文档；领域设计则引用父文档并承接分配。正文回答为什么做、做什么、如何工作及怎样验证，控制信息留文末。
...
**编写规范**：为每个关键直属对象写连续处理说明，而非只有职责表。父设计确定跨对象共同语义，不规定子系统内部全部线程、算法和私有字段。必要跨层约束标明来源与原因，下级文件未写不等于共同机制可以待下级自定。

**抽象示例**：处理子系统接收已校验输入，在分配额度内转换并返回候选结果；是否对外发布由系统既定规则决定。该子系统如何拆内部解析模块，由其概要设计展开。

**完成条件**：开发者知道每个对象接收什么、做什么、交给谁，无法解释的协作已经回到预设计，而非推给下游猜。
...
**编写规范**：下列过程可按实际能力扩展子节。每个关键判断追查事实由谁、从何来源、在何时产生；工程 Owner 不是运行统筹者。用相同 Process/Step/Constraint ID 引用机制细则，正文仍保留端到端原理和代表失败。
...
| Process ID / 模式 | 触发 / 目标 | 统筹者 / 参与方 | 前提事实来源 | 阶段 / 结果可见点 | 失败及清理 / 机制引用 |
|---|---|---|---|---|---|
...
输入提交 `dead7e7f64f97e87f096dadc2a790414185adb47`。独立模板 `design.software-system` 0.1.0，
