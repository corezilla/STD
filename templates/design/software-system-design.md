...
**必须写清楚**：外部及内部接口总表、提供/消费方、操作语义、数据类型、请求响应或事件、错误与兼容；关联目录、机器定义、实现及验证。
...
| Interface/Member ID / 角色 | 提供/消费对象及 backend | catalog / 唯一机器源 / selector | version/revision/hash | 语义 / 错误 / 兼容 | 实现位置 / 设计 V→Case/Run |
|---|---|---|---|---|---|
...
**本节目的**：确定哪些协议由软件系统决定，哪些必须继承或交给下级。

**必须写清楚**：系统公共接口、本层直属对象间接口、下级私有接口的唯一维护位置与变更责任；外部标准的版本和项目绑定选项。

**编写规范**：公共协议由系统或委托的唯一机制/契约维护，双方只分配实现。子系统私有接口留子系统，不上推全部细节。外部标准只复用适用条款，仍需确定项目对象、限制和错误行为；跨边界变更不得静默放宽保证。

**抽象示例**：SW-P 定义配置模块与处理子系统的交接；SUB-C 内部函数并不自动变成系统 API。第三方接口升级时检查实际使用的选项，不能只改版本号。

**完成条件**：每个边界只有一份语义 authority，公共与私有变更影响可追踪。
...
**本节目的**：验证接口是否真正能被双方调用和解释。

**必须写清楚**：触发前提、调用位置、目标及关联身份、参数类型与约束、跨字段条件、正常响应、错误、幂等范围、超时/取消及查询方式。事件接口说明生产消费和确认语义。

**编写规范**：按真实操作给完整请求/响应或事件实例，逐字段核对源定义和实例关联；不能只写字段名而缺必填、默认、单位、null 与错误语义。展示可从机器源生成，但不维护手抄的另一份字段真相；接口版本绑定文档既有封面和 metadata。

**抽象示例**：提交和查询实例必须使用同一任务身份；返回 accepted 仅代表准入，结果状态须由后续查询或事件判断。对于纯函数库则明确参数、返回值与异常，不强加任务协议。

**完成条件**：给定具体输入能走通双方处理；未知身份、非法组合、重复和迟到行为有明确结果。
...
**本节目的**：在系统设计阶段确定维护需要什么命令、从哪里执行及怎样控制影响。

**必须写清楚**：状态/版本/配置查询、指标日志获取、自检、受控操作、诊断导出、停止/恢复等实际命令类别；本地或远程入口、执行账号、目标对象、参数、返回和审计。

**编写规范**：逐命令明确执行主机、上下文、权限、作用域、确认和退出条件；危险动作有影响与回滚/停止边界。替代依赖、环回、旁路按调试能力说明，真实/模拟路径区分，不把测试钩子默认暴露给生产。指标含义在 §11 唯一维护。

**抽象示例**：诊断命令只能导出指定实例且经脱敏的数据；注入延迟的入口必须可查询命中、撤销并确认恢复，不是设置一个开关后默认所有请求受影响。

**完成条件**：运维知道在哪里、对谁执行及如何判断结束，调试能力可支持 §13 的受控验证而不越权。
...
**编写规范**：本章维护运行保证和判定语义，§9.3 维护访问接口，§7 维护端到端过程，§13 使用这些能力验证。不能用监控系统名称代替故障处理设计，也不能用自动重启掩盖数据丢失。
...
**编写规范**：验证项不是用例、环境或执行记录本身，按要求→设计验证项→Case→环境→Run 关联。测试设计使用 §9.3 的控制入口和 §11 的观测，不另造测试协议；所有适用能力及全部必需消费者进入分母。
...
**编写规范**：区分真实路径、替代依赖、注入与环回，替代依赖的接口边界在 §9.3 定义。每次演练分别检查结果已知性、访问安全、操作终态、资源释放、重新准入。有副作用且涉及取证与收口的机制演练“证据无法恢复但已安全停止”；只读机制检查中断、迟到响应与临时资源。
