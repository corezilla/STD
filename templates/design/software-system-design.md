...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.1.1 `InspectionState`（公共状态枚举，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.2.1 `InspectionRequest`（业务对象，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.2.2 `InspectionSubmission`（受理结果，虚构）**
...
- **约束与生命周期**：
  ...
  受理责任单元在任务身份持久确定后构造结果；响应丢失时仍可凭原 `request_id` 查询。任务何时完成由 §8.1 的 `InspectionState` 表达，不能从 `InspectionSubmission` 推断成功检测。
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.3.1 `config/inspection-policy.yaml`（配置文件，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.5.1 `CaptureStatusRegister`（设备寄存器，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.5.2 `RouteTableEntry`（FPGA 数据表项，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.6.1 `InspectionRuntimeState`（运行状态，虚构）**
...
- **Data/Type ID、用途与来源**：
  ...
  `DATA-EX-RUNTIME`；表示一项任务当前权威状态。示例机器源为 `Proposed: InspectionRuntimeState`；`InspectionState` 指向 §8.1 的同名教学类型，真实项目须改用自己的唯一来源。
...
- **`state`**：
  ...
  必填，取 §8.1 `InspectionState` 的已定义值。
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.8.1 `InspectionErrorCode`（公共错误码枚举，虚构）**
...
<!-- STD_TEMPLATE_EXAMPLE_BEGIN -->
**8.8.2 `InspectionError`（错误载荷结构，虚构）**
...
- **Interface/Member ID、用途、提供责任与唯一来源**:
`IF-EX-SUBMIT-INSPECTION`；受理一项图像检测请求，由 `InspectionAdmission` 提供。接口和三个类型均为教学用 `Proposed` 契约；输入与成功输出字段见 §8.2 的 `InspectionRequest`（`DATA-EX-REQUEST`）、`InspectionSubmission`（`DATA-EX-SUBMISSION`），错误码和载荷见 §8.8 的 `InspectionErrorCode`、`InspectionError`（`DATA-EX-ERROR-CODE`、`DATA-EX-ERROR`）。正式项目须换成固定的机器源、版本和成员 selector。

- **输入与前提**
	* `request`：必填,类型 `InspectionRequest`。其中 `request_id` 必须是本次业务请求的稳定身份；`image_uri` 必须指向调用身份有权读取的有效图像；`policy_version` 必须指向已发布规则。先检查身份权限，再检查请求字段、图像引用和规则版本；之后以 `request_id` 原子核对已受理记录：内容相同走重放，内容不同拒绝；只有新请求才检查队列容量并创建任务。字段的类型和跨字段约束只在 §8.2 定义，此处写该接口如何使用它们。
...
- **错误与合法下一步**：
输入字段或引用无效时返回 `InspectionError{code:INVALID_REQUEST, request_id:<原值或 null>, task_id:null}`，调用方修正请求；同一 ID 携带不同内容时返回 `REQUEST_CONFLICT`，调用方核对原请求，不换 ID 绕过；未获授权时返回 `PERMISSION_DENIED`，须先取得授权；新请求遇队列满返回 `QUEUE_FULL`，本次未受理，可稍后以同一请求身份再试。四种错误均不创建本次任务，码值含义和载荷约束由 §8.8 唯一维护。
