...
- **可自行选择/不可改变**：<!-- TODO -->
- **本地落实/内部再分配**：<!-- TODO -->
- **验证方法与结果/证据**：<!-- TODO / NOT_RUN -->
- **差距/变更影响/反馈责任**：<!-- TODO / none -->
...
- **类型 / 位置**：<!-- UI 页面 / CLI / HTTP-RPC 端点集 / 设备操作面；执行位置 -->
- **调用者 / 身份 / 被操作对象**：<!-- TODO -->
- **操作与入口**：<!-- TODO -->
- **输入与校验**：<!-- TODO -->
- **正常结果 / 可见性**：<!-- TODO -->
- **Empty / Error / Disabled / 取消**：<!-- TODO -->
- **§9 接口与 §11 维护引用**：<!-- TODO -->
...
**本节目的**：固定模块输入、工作区、输出及错误对象的权威关系。

**必须写清楚**：Data/Type ID、用途、public/private、唯一来源和寿命。

**编写规范**：只列真实使用对象，公共类型引用上级和机器源。不要逐结构维护完整调用方清单；流程与接口章节说明交互。

**抽象示例**：入口请求是公共类型，去重工作区是本模块私有类型，错误码引用系统定义。

**完成条件**：实现者能定位每个对象的字段定义和使用边界。

</details>

<!-- 编写建议：逐项列出模块拥有及引用的 Data/Type ID、用途、public/private 属性、唯一来源和固定版本。上级公共结构及公共错误码只引用原 ID；模块私有对象在 §6.2 定义。清单须覆盖入口、工作区、输出及失败载荷，不能只列业务实体。 -->

| Data/Type ID | 用途与范围 | 定义/引用 | 唯一来源与版本 | 生命周期 / 验证 |
|---|---|---|---|---|
...
**本节目的**：把模块拥有的关键类型展开到足以实现的字段深度。

**必须写清楚**：类型/引用、必填/空值/默认、范围/单位、条件有效性、键及不变量。

**编写规范**：公共结构只引用原 ID；私有工作区需写清真实字段及容器约束。教学图只示范表达，项目字段必须来自固定机器源或受控 Proposed 决定。

**抽象示例**：候选索引保存输入下标而非复制记录，越界下标应拒绝且不返回半成品。

**完成条件**：编码者无需自行决定关键字段和值域。

</details>

<!-- 编写建议：本模块拥有的结构逐字段说明类型或引用、必填/可空/默认、范围/单位、条件有效性及含义，给出键和跨字段不变量。公共类型由上级或机器源唯一维护；本节只说明必要的投影、转换和私有表示，不重写公共字段权威。 -->
...
- **本模块调用或消费**：<!-- TODO -->
- **本模块提供**：<!-- TODO -->
- **契约 authority / 版本 / selector**：<!-- TODO -->
- **同步方式 / timeout / 生命周期**：<!-- TODO -->
- **不可用或失败影响 / 责任出口**：<!-- TODO -->
...
- **运行载体与入口**：<!-- TODO / N/A + 依据 -->
- **并发/线程模型**：<!-- TODO / N/A + 依据 -->
- **初始化、Ready、生效与停止**：<!-- TODO / N/A + 依据 -->
- **宿主装配、失败和资源回收责任**：<!-- TODO / N/A + 依据 -->
...
<!-- 数据对象是独立多字段记录，统一使用以下固定格式；状态转换矩阵如需横向比较才另用短表。 -->

#### 6.2.N `<Data ID / Type>` · <数据结构名称>

- **Authority / 定义位置**：<!-- TODO -->
- **字段**：<!-- 字段名、类型、必填性、单位、范围、默认/空值语义 -->
- **键与跨字段约束**：<!-- TODO -->
- **Writer / Reader**：<!-- TODO -->
- **创建、持有、借用/复制与释放**：<!-- TODO -->
- **状态转换 / 并发规则**：<!-- TODO / N/A + 理由 -->
- **验证项**：<!-- TODO -->
...
**本节目的**：明确对象、引用与状态的实际持有和释放规则。

**必须写清楚**：创建、写入、借用/复制、可见、失效、释放及每条失败路径。

**编写规范**：对象寿命、业务状态和访问权是不同维度；不能用单一 Done 同时代表完成和释放。并发与取消条件要有来自接口或事件的真实证据。

**抽象示例**：函数返回独立结果；只读输入借用到实际返回结束，临时排序区在成功与拒绝时都释放。

**完成条件**：从任一出口都不存在悬空引用、重复释放或无主事实。

</details>

<!-- 编写建议：对 §6.1 每个关键对象说明创建、唯一写者、读取、借用/复制、可见、失效和释放；列合法状态转换、Guard 的事实来源及并发/取消/迟到处理。私有状态规则不得改变上级可观察行为；没有持久化仍需说明在途对象寿命。 -->

### 6.4 示例与验证

<details><summary>本节编写建议、规范与示例</summary>

**本节目的**：用输入向量检验模块字段、不变量和生命周期是否可编码。

**必须写清楚**：合法与拒绝实例、预期结果和错误、来源基线及独立 V/Case。

**编写规范**：至少覆盖一项跨字段或状态条件，实例必须与 §7 流程和 §9 接口一致。示例不是已运行证据，未实现和未执行要分开。

**抽象示例**：第 4097 条记录被入口拒绝，无候选数组泄露；4096 条合法输入按原顺序返回。

**完成条件**：编码者与测试者能对同一向量得到一致的接收、拒绝和清理结果。

</details>

<!-- 编写建议：对模块拥有的关键输入、工作对象、输出及错误载荷给合法与边界/拒绝实例；逐项核对字段和不变量、状态与寿命，引用 §14 的独立 V/Case。已有机器定义的实例固定来源；未实现和未运行分别标注，不能以教学图例充当项目验证。 -->
...
- **Interface/Member ID、用途、提供责任与来源**：<!-- 继承/本层拥有/私有；固定版本、selector/hash -->
- **输入与前提**：<!-- 参数/请求 Data/Type ID、校验顺序、身份权限、ownership -->
- **成功输出与保证**：<!-- 返回/响应 Data/Type ID、状态、副作用与寿命 -->
- **错误与合法下一步**：<!-- 逐条件 Error ID、载荷、结果已知性、查询/重放/停止 -->
- **交互与生命周期**：<!-- 上下文、同步/异步、并发、超时/取消、版本、释放 -->
- **实现与验证**：<!-- 真实文件/symbol 或 Planned、合法与拒绝实例、V/Case/Run -->
...
- **输入、输出及关联身份**：<!-- Data/Type ID、编码、消息边界、确认 -->
- **交互、错误及生命周期**：<!-- 顺序、流控、超时、重复、取消、释放 -->
- **实现与验证**：<!-- 位置/状态、正常及异常实例、V/Case/Run -->
...
- **执行位置、目标与输入**：<!-- 环境、身份、参数或控件、前置 -->
- **输出、错误与交互**：<!-- 反馈、超时/取消、作用范围、审计、退出 -->
...
- **初始条件 / 并发交错 / 失败点**：<!-- TODO -->
- **检测事实 / authority / 期限**：<!-- TODO -->
- **处理行为 / 副作用边界**：<!-- TODO -->
- **状态查询 / 同请求重放 / 接管 / 新业务重试**：<!-- TODO / N/A + 理由 -->
- **最终状态 / 资源归属 / 后续合法入口**：<!-- TODO -->
- **验证项 / 组合责任**：<!-- TODO -->
...
- **目标 / 限制 / 单位**：<!-- TODO -->
- **适用版本 / 配置 / 硬件 / 虚拟化 / 依赖**：<!-- TODO -->
- **负载、数据规模与并发口径**：<!-- TODO -->
- **推导 / 测量方法与证据等级**：<!-- Modeled / Simulated / Measured -->
- **共享资源扣减 / 峰值重叠 / 余量**：<!-- TODO -->
- **超限行为 / 责任出口**：<!-- TODO -->
- **验证项 / Evidence**：<!-- TODO / NOT_RUN -->
...
- **职责 / 非职责**：<!-- TODO -->
- **关键 symbol / 导出范围**：<!-- TODO -->
- **承接 Function / Rule / Constraint / Interface ID**：<!-- TODO -->
- **构建目标 / 依赖 / 宿主装配**：<!-- TODO -->
...
#### 14.N `VRC-<MODULE>-<nnn>` · <验证要求名称>

- **覆盖 Function / Rule / Constraint / Interface**：<!-- TODO -->
- **Case / 正常、边界与失败输入**：<!-- TODO -->
- **环境 / 配置 / 隔离与复位**：<!-- TODO -->
- **独立 Oracle / Expected**：<!-- TODO -->
- **Actual / Evidence / Run ID**：<!-- TODO / NOT_RUN -->
- **Verdict / 状态**：<!-- NOT_RUN / BLOCKED / FAIL / PARTIAL / Verified -->
- **父级组合验证交接**：<!-- TODO -->
...
- **采用模式**：<!-- embedded / separate / not_required / missing -->
- **模块对象 ID**：<!-- TODO -->
- **实现规格 Document ID**：<!-- embedded 时为本文 Document ID；separate 时为独立 ISD -->
- **metadata 覆盖映射入口**：<!-- §5/§6/§9/§10/§13/§14 的实际稳定锚点或覆盖清单 -->
- **理由 / 决定引用**：<!-- embedded 仅在本文已覆盖 ISD 必需信息时采用；覆盖不足时改为 separate 或 missing -->
...
- **类型 / 影响的规则、接口、流程或约束**：<!-- Risk / Open Question / Decision -->
- **事实缺口 / 触发条件**：<!-- TODO -->
- **影响 / 阻塞边界**：<!-- TODO -->
- **Owner / 最晚关闭 Gate**：<!-- TODO -->
- **选项 / 推荐 / 下一步取证**：<!-- TODO -->
- **关闭条件 / 决定或当前状态**：<!-- TODO -->
...
- **来源 Capability / Step / Constraint / 接口成员**：<!-- TODO -->
- **本模块必须负责的行为与保证**：<!-- TODO -->
- **本模块提供 / 消费的接口**：<!-- TODO -->
- **本文落实位置**：<!-- TODO -->
- **代码文件 / symbol 或 NOT_IMPLEMENTED**：<!-- TODO -->
- **允许自行决定的范围**：<!-- TODO -->
- **本地验证 / 组合验证交接**：<!-- TODO -->
