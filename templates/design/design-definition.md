...
<!-- 记录私有实现交接；公共接口仍以 §9 的唯一机器源为 authority。 -->
...
**接口与错误承接**：先列模块提供/消费的全部 Interface/Member ID、请求/响应/错误类型、固定签名/来源和实现责任；每个失败分支引用系统 Error ID 或私有码的边界映射，写明载荷、结果状态、副作用与调用方动作。§5.3 仅引用同一成员 ID 描述文件间交接。
...
<!-- 接口与成员映射都是独立多字段记录；逐项使用以下固定格式，不压缩签名和错误语义。 -->

#### 9.N `<Interface ID>` · <接口或操作名称>

- **Direction / Operation / 责任模块 / backend**：<!-- TODO -->
- **Request / Response / Error / ownership**：<!-- TODO -->
- **Contract authority / version / revision / hash / selector**：<!-- TODO -->
- **前提 / timeout / 兼容边界 / Error model**：<!-- TODO -->
- **本地文件 / symbol 或 NOT_IMPLEMENTED**：<!-- TODO -->
- **Constraint / VRC / Case / 环境 / Run**：<!-- TODO -->
- **关联类型字段 ID**：<!-- TODO -->
