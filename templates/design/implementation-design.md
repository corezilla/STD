...
列出实现视图承接及新增的 Data/Type/Error ID、定义/引用关系、唯一来源、固定基线和文件/symbol。清单只定位权威；已有公共字段及公共错误语义不得在 ISD 改写。无新类型也列承接对象与适用性依据；完成时每个实现对象都能定位字段来源和验证入口。

</details>

<!-- 编写建议：列出对应模块设计所拥有及引用的 Data/Type/Error ID、固定来源和版本、ISD 是否新增私有表示、落地文件/symbol。公共字段与错误语义只继承，不在代码级视图重新决定；缺少上级结构时回报设计缺口。 -->

| Data/Type ID | 定义/引用与用途 | 固定来源 / 版本 | 私有表示或文件/symbol | 验证入口 |
|---|---|---|---|---|
...
只展开本 ISD 拥有的私有语言类型，逐字段给类型、初值/空值、范围/单位、条件有效性和不变量，并解释与上级公共类型的投影。机器源已有字段只固定版本与 selector，不复制另一套权威；未知语义退回上级设计，不让编码者猜。完成时字段足以构造合法及拒绝实例。

</details>

<!-- 编写建议：只完整定义本 ISD 拥有的私有类型与语言级表示：逐字段给类型、初值/可空/范围/单位、不变量和条件有效性，说明与上级公共类型的映射。已有公共机器定义只固定来源并核对代码投影，不形成第二份字段 authority。 -->
...
将上级寿命与状态约束落实为创建/修改/读取函数、借用范围、锁或事务参与、可见/提交点、清理函数及错误出口。不能以代码路径存在推断已验证；对中断、崩溃和迟到结果分别检查有效对象及释放。完成时每条正常和失败路径均可走到有主的终态。

</details>

<!-- 编写建议：对 §4.1 的实现对象逐项标明创建/修改/读取函数、持有与借用期限、可见和提交点、释放及失败出口；把上级状态约束落实到具体锁、事务或原子操作。无持久状态时仍检查临时对象和在途调用；持久化与崩溃恢复的函数落点在 §7.2–§7.3 深化。 -->

### 4.4 示例与验证

<details><summary>编写要求与完成条件</summary>

给私有表示或公共类型投影的合法及边界/拒绝实例，说明逐字段 Expected、独立 Oracle、错误分支、资源结果和 V/Case/Run 状态。实例从公开入口或受控依赖故障点推演，不直接篡改业务终态；未运行保持 NOT_RUN。完成时编码与测试双方能复算同一结果。

</details>

<!-- 编写建议：提供一条合法实例和一条边界/拒绝实例，逐字段验证私有表示与公共类型的映射、所有权、错误出口及释放；绑定 §9 的独立 Oracle、V/Case 和实际执行状态。不能以示意图或直接改业务终态冒充运行证据。 -->

### 4.5 语言级表示与代码映射

<details><summary>编写要求与完成条件</summary>

逐 Data/Type ID 说明真实文件、类型和函数 symbol、序列化/转换点、构建目标及失败映射；Planned 位置只留在正文，机器目录未实现 location/symbol 仍为 null。代码变更应与固定机器源双向核对。完成时开发者能定位每个实际字段从输入到代码到输出的唯一落点。

</details>

<!-- 编写建议：记录 Data/Type ID 到真实文件、类型/symbol、序列化/转换函数和构建目标的精确映射；Planned 路径只用于正文计划，机器目录的未实现 location/symbol 保持 null。说明字段保持、补足或损失及失败映射，代码变更时与唯一机器来源双向核对。 -->
...
- **Interface/Member ID、用途**：<!-- TODO -->
- **文件 / symbol / 可见性**：<!-- 真实位置；Planned 不冒充代码 -->
- **原成员 ID 或私有来源**：<!-- TODO -->
- **完整签名与 caller**：<!-- 真实签名、代表调用上下文；不维护所有调用方反向列表 -->
- **固定契约与版本**：<!-- selector/revision/hash、构建目标 -->
- **输入参数 / 数据结构 authority**：<!-- 逐参数 ID、类型、必填、范围、单位/编码、ownership 与寿命 -->
- **输入约束 / 校验顺序 / 失败映射**：<!-- 跨字段不变量、校验 symbol、失败对应 Error ID -->
- **成功输出 / 数据结构 / 后置条件**：<!-- 互斥返回变体、字段不变量、状态与副作用、ownership 与寿命 -->
- **错误输出 / 触发条件 / 优先级**：<!-- 逐 Error ID、类型/代码、稳定 message/payload、结果已知性与合法下一步 -->
- **底层异常 / 失败事实**：<!-- 本接口内每个关键错误的原始事实 -->
- **模块是否处理及处理函数**：<!-- propagate / translate / recover / reject，实际 symbol -->
- **Typed 异常与原生异常所有权**：<!-- 谁抛出、谁捕获、谁映射 -->
- **宿主 / public payload 或状态码**：<!-- 实际返回及 Error ID -->
- **日志级别 / 脱敏 / 关联字段**：<!-- TODO -->
- **是否可重试及前提**：<!-- query / same-request replay / takeover / new attempt -->
- **状态与副作用影响 / 验证项**：<!-- TODO -->
- **不可改变的规则 / Constraint ID**：<!-- 上级行为、不变量、预算和错误语义 -->
- **实现自由度**：<!-- 私有表示、helper、库或局部优化边界 -->
- **副作用 / 执行上下文 / 幂等性**：<!-- TODO -->
- **输入输出 ownership 与寿命**：<!-- TODO -->
- **Thread-safe / reentrant**：<!-- yes/no/conditional -->
- **Nested-call policy**：<!-- allowed/forbidden；锁与回调边界 -->
...
- **装配、合法及拒绝实例**：<!-- 初始化位置；独立 Oracle/Expected、Actual/Evidence、Verdict、V/Case/Run -->
...
- **Interface/Member ID、处理文件/symbol 与来源**：<!-- TODO -->
- **输入、输出和字段校验**：<!-- Data/Type ID、编码、边界、关联身份 -->
- **交互、错误传播与寿命**：<!-- 顺序/确认/积压、逐异常映射、释放 -->
- **实例与验证**：<!-- 合法及拒绝向量、V/Case/Run -->
...
- **Interface/Member ID、文件/symbol、端点与来源**：<!-- TODO -->
- **输入输出与硬件约束**：<!-- 方向、位宽、电气、时钟复位、时序 -->
- **错误、恢复与寿命**：<!-- 失联/超时/非法状态、清理 -->
...
- **Interface/Member ID、文件/symbol 与来源**：<!-- TODO -->
- **执行位置、目标、输入与权限**：<!-- 参数/控件、校验位置 -->
- **输出、错误与交互**：<!-- 反馈/退出码、日志/脱敏、取消/恢复 -->
- **实例与验证**：<!-- 合法及拒绝调用、V/Case/Run -->
