...
**编写规范**：实例至少走过一处参与方边界，核对身份、版本及所有权。调用演练仍在 §5.1 完成，本节只核对数据，不复制完整接口签名。给出可复算的字段取值、拒绝条件和预期状态，逐项关联机器源与设计 V/Case；不能把虚构实例或局部 PASS 宣称为真实组合验证。
...
**接口与错误承接**：先列全部 Operation/Member ID、提供/调用方、固定签名及 request/response/error 类型；逐成员描述成功、失败和结果已知性。错误引用系统 Error ID 并说明转换或透传，下级分配见 §14.3。
