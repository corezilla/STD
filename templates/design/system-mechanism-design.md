...
本模板名称为“总体系统机制设计”，沿用 Template ID `design.system-mechanism` 与现有文件路径。
机制横向承接总体系统的公共行为，可跨软件、固件和硬件责任单元；不代表分解树中的固定一级。
...
正文与编写过程隔离：模板选择、填写步骤、绘图指令和修改经过留在帮助或评审记录中，正式开篇直接介绍实际机制。生成后移除教学块并折叠帮助，逐段复查正文及图注，不以模板结构检查代替内容判断。
...
<!-- 在此说明类型阅读基线（catalog、源版本/revision/hash、正文锚点）。 -->
