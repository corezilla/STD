...
<!-- 在此写入机制数据结构的共同设计结论；§4.1–§4.6 分别给出索引、定义、寿命、实例与跨参与方语义。 -->

### 4.1 数据结构清单

<details><summary>本节编写建议、规范与示例</summary>

**本节目的**：建立跨参与方对象、状态和错误载荷的共同索引。

**必须写清楚**：Data/Type ID、用途、来源基线、生产与消费边界、定义或引用关系。

**编写规范**：清单只定位唯一权威，不把协议字段复制成第二份定义。机制没新增结构时列出实际承接对象和正式裁剪依据。对每个跨边界对象核对生产和消费两侧引用的来源版本、身份范围及条件有效性；未定字段不得用“模块再细化”掩盖共同契约缺口。

**抽象示例**：请求与响应引用同一 operation_id 类型，局部采样对象只由目标单元定义。

**完成条件**：每个跨边界对象都能定位契约，参与方不会自行解释同名字段。

</details>

<!-- 编写建议：列出机制拥有及引用的状态、消息、错误载荷 Data/Type ID、用途、唯一来源和固定版本；跨参与方交接只引用共同定义。没有新结构也要列承接对象及裁剪依据。 -->
...
**本节目的**：固定交接过程中谁拥有事实、何时可见及何时安全释放。

**必须写清楚**：创建、写入、读取、提交/可见、失效、释放、在途关联及失败后保证。

**编写规范**：状态引用 §4.1 相同 ID，并与 §6–§9 的流程和异常一致。区分局部一致与全局一致，不把响应返回推断为持久化。逐个在途对象检查取消、超时、迟到和故障后谁仍有写入权限，何种证据允许释放，以及无法取证时保持什么安全状态。

**抽象示例**：采样响应已返回，只能证明目标在该时刻形成局部快照，不能证明所有目标同时更新。

**完成条件**：正常、迟到和失败交接都能确定唯一事实来源及资源归属。

</details>

<!-- 编写建议：对 §4.1 中每个关键对象说明创建者、唯一写者、可见/提交点、读者、失效与释放责任；按实际区分局部一致、跨参与方一致、结果已知性和持久保证。对消息交接说明关联身份与在途所有权；详细故障范围在 §4.6 推演，不把“已返回”误写成“已持久”。 -->

### 4.4 示例与验证

<details><summary>本节编写建议、规范与示例</summary>

**本节目的**：用可推演的实例暴露字段、交接和错误语义的缺口。

**必须写清楚**：合法及边界/拒绝对象、条件判定、编码基线、V/Case 和验证状态。

**编写规范**：实例至少走过一处参与方边界，核对身份、版本及所有权。调用演练在 §5 对应的逐接口记录内完成，本节只核对数据，不复制完整接口签名。给出可复算的字段取值、拒绝条件和预期状态，逐项关联机器源与设计 V/Case；不能把虚构实例或局部 PASS 宣称为真实组合验证。

**抽象示例**：迟到响应带旧 generation，接收方拒绝将其写入当前报告，并保留可诊断关联 ID。

**完成条件**：双方独立按同一实例判定接受、拒绝和资源去向。

</details>

<!-- 编写建议：对机制拥有的关键请求、状态或载荷给一条合法实例及一条边界/拒绝实例，核对字段、条件有效性、编码、交接和寿命，并绑定 §15 的设计 V/Case。已继承的类型引用权威实例；未实现、未执行应分别标注。 -->
...
- **Interface/Member ID、用途与提供责任**：<!-- 稳定 ID；本机制拥有或引用 -->
- **唯一契约、版本与状态**：<!-- 机器源 selector/revision/hash；Proposed/Implemented/Verified 分开 -->
- **输入与前提**：<!-- 参数或请求 Data/Type ID、目标身份、授权、校验顺序 -->
- **成功输出与保证**：<!-- 响应/事件 ID；accepted/effective/completed/durable/released 的适用范围 -->
- **错误与合法下一步**：<!-- 逐条件 Error ID、结果已知性、副作用、查询/等待/安全重试条件 -->
- **交互与生命周期**：<!-- 同步/异步、超时、取消、重复/幂等、并发、资源释放、版本 -->
- **代表调用与验证**：<!-- 正常及失败/取消实例，同一对象身份、V/Case、Run 状态 -->
...
- **输入与输出**：<!-- 消息/帧 Data/Type ID、编码、字段约束、确认或响应 -->
- **错误与交互**：<!-- 顺序、积压/背压、重复、期限、取消、丢失、释放及合法下一步 -->
- **实例与验证**：<!-- 正常及异常消息、V/Case/Run -->
...
- **信号与输入输出**：<!-- Data/Type ID、方向、连接/管脚、位宽、电气、编码 -->
- **交互与错误**：<!-- 时钟复位、握手、期限、失联、错误、状态及安全释放 -->
...
- **Interface/Member ID、用途、提供责任与唯一来源**：<!-- TODO -->
- **执行位置、目标与输入**：<!-- 环境、权限、参数或控件、前置状态 -->
- **输出、错误与交互**：<!-- 反馈、退出码、超时/取消、作用范围、审计及恢复 -->
